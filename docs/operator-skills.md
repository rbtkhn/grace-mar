...
|------|---------|-----------------|
| `coffee` | **Step 1** (work-start: warmup + harness + branch snapshot + **Lane context (for hub B / D)** — **no** internet intel; **no** daily brief generator; **signing-off:** `operator_coffee.py --mode closeout` or `operator_handoff_check.py` + summary) + **Step 2** **same** **A–E** hub (**no** micro-hints row under the list). **A — Steward:** gate \| template \| integrity \| **git/ship** (`A integrity`, `A git`, …). **B — Engineer:** work-dev + skills/meta — **not** git/ship (**A git**) or membrane-only audits. **C — Historian:** exactly three options only — **A Intel**, **B Bookshelf quiz → IX-A candidates**, **C Notebook synthesis with Tri-Frame**. **D — Capitalist:** work-business / grace-gems / commercial bookshelf/product slices. **E — Conductor:** continuation after **`coffee`**; standalone **`conductor`** / master name without **`coffee`** also valid. **C** exits unless **`stay in coffee`**; **A**, **B**, **D**, **E** re-offer full hub; **A** uses steward fork when actionable. **Signing-off:** **A** without steward sub-track → **system pick**. See [polling-and-markets.md](skill-work/work-politics/polling-and-markets.md), `.cursor/skills/coffee/SKILL.md`, [menu-reference — signing-off](skill-work/work-coffee/menu-reference.md#signing-off-intent). | **`operator_coffee.py`** (modes) + agent steps |
