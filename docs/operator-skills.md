...
| Skill | Say this | Note |
|------|----------|------|
| `coffee` | **coffee** | Canonical cadence skill; **closeout** = same **coffee** with signing-off intent (handoff Step 1). Legacy **hey** still works. |
...
1. Start with `coffee` when opening a new work block or a new agent thread. The operator may say **`coffee` more than once per day** for reorientation; each invocation runs **Step 1** again (see `.cursor/skills/coffee/SKILL.md` § *Multiple coffees per day*). Legacy **`hey`** still works. Use **coffee light** / **minimal** for lighter repeat passes. On **work-start `coffee`**, the agent runs **Step 1** (scripts + branch snapshot — **no** Polymarket / poll web / Massie X in Step 1), then **Step 2**: fixed **A–H** — **E → work-politics** runs KY-4 + Polymarket + polls + Massie X per [polling-and-markets.md](skill-work/work-politics/polling-and-markets.md) when not survey-only; **C** runs brief generator + §1d Putin web **only when chosen**; **G** / **H** per skill table. **Concrete work-lane picks** like **E** with a sub-lane or **G** exit to normal workflow unless the operator says **`stay in coffee`** — see `.cursor/skills/coffee/SKILL.md` § *Coffee — multiple choice (A–H required)*. **Cadence:** Monday = full routine; Tue–Fri = lighter; Sunday ≈ 10 min week-ahead (FEC, registration, brief registry); Friday adds a two-line post-mortem — see `.cursor/skills/coffee/SKILL.md` § Cadence by weekday. On **closeout `coffee`** (session end / signing off — **same trigger**), **Step 1** = `operator_handoff_check.py` + summary, then **Step 2** = **closeout A–H** — **A–D** match work-start (**D** = RECURSION-GATE); **E** = system pick; **G** / **H** shared — see `.cursor/skills/coffee/SKILL.md` § *Coffee — closeout*, § *Shared A–C*, § *Shared D*, § *Shared G-H*. **F** closes only the current pass, not the calendar day.
