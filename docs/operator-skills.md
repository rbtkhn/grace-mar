...
| Skill | Say this | Note |
|------|----------|------|
| `operator-cadence` | **hey** | One skill; **closeout** = same **hey** with signing-off intent (handoff Step 1). |
...
**Skill discovery:** Pointer backlog [skills-portable/skill-candidates.md](../skills-portable/skill-candidates.md), draft lane `skills-portable/_drafts/`, ladder in [skills-portable/README.md](../skills-portable/README.md). After substantive **EXECUTE** / **DOCSYNC** ships, optional one-line prompt per [.cursor/rules/operator-style.mdc](../.cursor/rules/operator-style.mdc) (Skill discovery). **Hey** menu **H** is the dedicated skills slot; **handoff-check** summary may mention the same.

---

## Suggested daily pattern

1. Start with `operator-cadence` when opening a new work block or a new agent thread. The operator may say **“hey” more than once per day** for reorientation; each invocation runs **Step 1** again (see `.cursor/skills/operator-cadence/SKILL.md` § *Multiple heys per day*). Use **hey light** / **minimal** for lighter repeat passes. On **work-start “hey”**, the agent runs **Step 1** (scripts + branch snapshot — **no** Polymarket / poll web / Massie X in Step 1), then **Step 2**: fixed **A–H** — **E → work-politics** runs KY-4 + Polymarket + polls + Massie X per [polling-and-markets.md](skill-work/work-politics/polling-and-markets.md) when not survey-only; **C** runs brief generator + §1d Putin web **only when chosen**; **G** / **H** per skill table — see `.cursor/skills/operator-cadence/SKILL.md` § *Hey — multiple choice (A–H required)*. **Cadence:** Monday = full routine; Tue–Fri = lighter; Sunday ≈ 10 min week-ahead (FEC, registration, brief registry); Friday adds a two-line post-mortem — see `.cursor/skills/operator-cadence/SKILL.md` § Cadence by weekday. On **closeout “hey”** (session end / signing off — **same trigger**), **Step 1** = `operator_handoff_check.py` + summary, then **Step 2** = **closeout A–H** — **A–D** match work-start (**D** = RECURSION-GATE); **E** = system pick; **G** / **H** shared — see `.cursor/skills/operator-cadence/SKILL.md` § *Hey — closeout*, § *Shared A–C*, § *Shared D*, § *Shared G-H*. **F** closes only the current pass, not the calendar day.
...
6. End the day with **closeout hey**: **`handoff-check`** output is **Step 1**; agent then shows **closeout A–H** (**F** = formally end closeout). To resume mid-thread without full closeout flow, use `handoff-check` alone per `.cursor/skills/handoff-check/SKILL.md`.
