...
| Skill | Say this | Note |
|------|----------|------|
| `operator-cadence` | **coffee** | One skill; **closeout** = same **coffee** with signing-off intent (handoff Step 1). Legacy **hey** still works. |
...
|------|---------|-----------------|
| `operator-cadence` | **Work-start coffee:** **Step 1** (warmup + harness + branch snapshot — **no** internet intel; **no** daily brief generator; **menu C** for brief + §1d Putin web) + **Step 2** **A–H** (order **A,B,C,D,E,G,H,F**; **F** ends session). **Concrete work-lane picks** like **E** with a sub-lane or **G** exit to normal workflow by default unless the operator says **`stay in coffee`**. **Closeout coffee** (same trigger **coffee**, signing-off intent; legacy **hey** still works): **Step 1** `operator_handoff_check.py` + summary + branch snapshot + **Step 2** **A–H** — **A–D** same as work-start (**D** = gate); **E** system pick; **G** / **H** same; **F** end closeout. **KY-4 + Polymarket + polls + Massie X:** **menu E → work-politics** (or explicit same-message request); **Companion survey** path skips that stack unless asked. **G** = work-jiang / Predictive History; **H** = skills / meta pipeline. See [polling-and-markets.md](skill-work/work-politics/polling-and-markets.md) and `.cursor/skills/operator-cadence/SKILL.md` § *Shared A–C* / § *Shared D* / § *Shared G-H* | **Work-start:** `operator_daily_warmup.py` + agent steps; **closeout:** `operator_handoff_check.py` + agent steps |
...
**Skill discovery:** Pointer backlog [skills-portable/skill-candidates.md](../skills-portable/skill-candidates.md), draft lane `skills-portable/_drafts/`, ladder in [skills-portable/README.md](../skills-portable/README.md). After substantive **EXECUTE** / **DOCSYNC** ships, optional one-line prompt per [.cursor/rules/operator-style.mdc](../.cursor/rules/operator-style.mdc) (Skill discovery). **Coffee** menu **H** is the dedicated skills slot; **handoff-check** summary may mention the same.

---

## Suggested daily pattern

1. Start with `operator-cadence` when opening a new work block or a new agent thread. The operator may say **`coffee` more than once per day** for reorientation; each invocation runs **Step 1** again (see `.cursor/skills/operator-cadence/SKILL.md` § *Multiple coffees per day*). Legacy **`hey`** still works. Use **coffee light** / **minimal** for lighter repeat passes. On **work-start `coffee`**, the agent runs **Step 1** (scripts + branch snapshot — **no** Polymarket / poll web / Massie X in Step 1), then **Step 2**: fixed **A–H** — **E → work-politics** runs KY-4 + Polymarket + polls + Massie X per [polling-and-markets.md](skill-work/work-politics/polling-and-markets.md) when not survey-only; **C** runs brief generator + §1d Putin web **only when chosen**; **G** / **H** per skill table. **Concrete work-lane picks** like **E** with a sub-lane or **G** exit to normal workflow unless the operator says **`stay in coffee`** — see `.cursor/skills/operator-cadence/SKILL.md` § *Coffee — multiple choice (A–H required)*. **Cadence:** Monday = full routine; Tue–Fri = lighter; Sunday ≈ 10 min week-ahead (FEC, registration, brief registry); Friday adds a two-line post-mortem — see `.cursor/skills/operator-cadence/SKILL.md` § Cadence by weekday. On **closeout `coffee`** (session end / signing off — **same trigger**), **Step 1** = `operator_handoff_check.py` + summary, then **Step 2** = **closeout A–H** — **A–D** match work-start (**D** = RECURSION-GATE); **E** = system pick; **G** / **H** shared — see `.cursor/skills/operator-cadence/SKILL.md` § *Coffee — closeout*, § *Shared A–C*, § *Shared D*, § *Shared G-H*. **F** closes only the current pass, not the calendar day.
...
6. End the day with **closeout coffee**: **`handoff-check`** output is **Step 1**; agent then shows **closeout A–H** (**F** = formally end closeout). To resume mid-thread without full closeout flow, use `handoff-check` alone per `.cursor/skills/handoff-check/SKILL.md`.
...
**Legacy:** Cursor skill folder/id was `daily-warmup` (renamed for cadence/tempo). Update bookmarks to `.cursor/skills/operator-cadence/`.
...
- **Coffee:** **Polymarket** + **volume** + **independent** horserace poll (or none) — **only** after **menu E → work-politics** (intel path) or explicit same-message request; same caveats and procedure as [polling-and-markets.md](skill-work/work-politics/polling-and-markets.md). Step 1 does **not** include this block.
- **Coffee:** After **Step 1**, fixed **A–H** menu (present **A,B,C,D,E,G,H,F**) — **A** template + boundary audit combined (`template_diff.py` / audit reports + boundary / fork isolation / [skills-modularity.md](skills-modularity.md); optional `validate-integrity.py`; **required closing:** upstream/downstream **reconciliation code** recommendations per [work-companion-self/README — Reconciliation code audit](skill-work/work-companion-self/README.md#reconciliation-code-audit-upstream-and-downstream)), **B** repository hygiene (git branches, status, commit/push plan, exports/integrity per operator-cadence table), **C** Daily brief (`generate_work_politics_daily_brief.py` + §1d Putin in-file — **only when C is chosen**; Step 1 does not generate), **D** RECURSION-GATE (`recursion-gate.md`, gate-review-pass; **no merge** without companion approval), **E** work-dev *or* work-strategy *or* work-politics — **E / work-politics** = KY-4 intel stack when not survey-only; survey defaults under **E** / work-politics when `coffee survey` **without** forced Polymarket/Massie unless asked; **G** work-jiang / Predictive History; **H** skills / meta pipeline; **F** end coffee session → normal workflow. Concrete work-lane picks under **E** or **G** exit to normal workflow by default unless the operator says **`stay in coffee`**. See `.cursor/skills/operator-cadence/SKILL.md` § *Coffee — multiple choice (A–H required)*.
- **Closeout coffee:** After **Step 1** (`operator_handoff_check.py` + paragraph), fixed **A–H** — **A–D** same as work-start (template + boundary, repository hygiene, Daily brief when chosen, **D** RECURSION-GATE), **E** system pick (integrity, survey, commit grouping, etc.; prefer **G** for Jiang/PH, **H** for skills), **G** / **H** same as work-start; **F** formally end closeout. See `.cursor/skills/operator-cadence/SKILL.md` § *Shared A–C*, § *Shared D*, § *Shared G-H*, and § *Coffee — closeout — multiple choice (A–H required)*.
