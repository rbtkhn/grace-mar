...
| Skill | Say this | Note |
|------|----------|------|
| `coffee` | **coffee** | Canonical cadence skill; **signing-off** intent = handoff-weighted Step 1, **same** **A–D2–E** menu (Build, Steward, Strategy/daily brief, Conductor continue, Conductor system, system choice — **B** alone after signing-off = system pick when no sub-track). Legacy **hey** still works. |
...
6. End the day with **`coffee`** + **signing-off** intent: **`handoff-check`** (or `operator_coffee.py --mode closeout`) is **Step 1**; agent then shows the **same** **A–D2–E** menu as work-start (**B — Steward** alone = system pick). To resume mid-thread without the full `coffee` flow, use `handoff-check` alone per `.cursor/skills/handoff-check/SKILL.md`.
