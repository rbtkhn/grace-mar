...
| Skill | Say this | Note |
|------|----------|------|
| `coffee` | **coffee** | Canonical cadence skill; **signing-off** intent = handoff-weighted Step 1, **same** **A–D** hub (**A** Build, **B** Steward, **C** Strategy daily brief, **D** system choice). **Symphony conductors** are **not** on the hub — call **`conductor`** or a **master** name separately. **B** alone after signing-off = system pick when no sub-track. Legacy **hey** still works. |
...
|------|---------|-----------------|
| `coffee` | **Step 1** (work-start: warmup + harness + branch snapshot + **Lane context (for menu D)** — **no** internet intel; **no** daily brief generator; **signing-off:** `operator_coffee.py --mode closeout` or `operator_handoff_check.py` + summary) + **Step 2** **same** **A–D** hub (**no** micro-hints row under the list). **A — Build:** work-dev + skills/meta — **not** git/ship (**B git**) or membrane-only audits. **B — Steward:** gate \| template \| integrity \| **git/ship** (`B integrity`, `B git`, `B both`, `B all` when explicit). **C — Strategy (daily brief):** brief + §1d Putin + §1e JD Vance + optional KY-4 + Tri-Frame minds (survey defaults here with **`coffee survey`**). **D — (system choice):** one line (quiz, **skill-write** / PRP, Cici next, Dev next, Rome, Jiang/PH, …). **Conductors:** [conductor skill](../../.cursor/skills/conductor/SKILL.md) / master name — **not** a hub letter. **C** exits to normal workflow unless **`stay in coffee`**; **A**, **B**, **D** re-offer full hub by default; **B** uses steward fork when actionable. **Signing-off:** **B** without steward sub-track → **system pick**. See [polling-and-markets.md](skill-work/work-politics/polling-and-markets.md), `.cursor/skills/coffee/SKILL.md`, [menu-reference — signing-off](skill-work/work-coffee/menu-reference.md#signing-off-intent). | **`operator_coffee.py`** (modes) + agent steps |
...
1. Start with `coffee` when opening a new work block or a new agent thread. **Mid-day pause / depth:** prefer a **Conductor** pass (`.cursor/skills/conductor/SKILL.md`) or **`coffee` light/minimal**; **`thanks`** is **deprecated** as a habit (see `.cursor/skills/thanks/SKILL.md`). The operator may say **`coffee` more than once per day** for reorientation; each invocation runs **Step 1** again (see `.cursor/skills/coffee/SKILL.md` § *Multiple coffees per day*). Legacy **`hey`** still works. Use **coffee light** / **minimal** for lighter repeat passes. On **work-start `coffee`**, the agent runs **Step 1** (scripts + branch snapshot + **Lane context (for menu D)** — **no** Polymarket / poll web / Massie X in Step 1), then **Step 2**: **A–D** hub — **A — Build**; **B — Steward** (gate / template / integrity / **git/ship**); **C — Strategy (daily brief)** runs brief + §1d Putin + §1e JD Vance + optional KY-4 intel per [polling-and-markets.md](skill-work/work-politics/polling-and-markets.md) when not survey-only; **D — (system choice)** (one line per session). **Symphony conductors:** separate (**`conductor`**, master name). **C** exits to normal workflow unless **`stay in coffee`**; **A**, **B**, **D** re-offer the full hub by default; **B** uses the steward fork (**Implement now** / **Later**) when actionable — see [menu-reference.md](skill-work/work-coffee/menu-reference.md#steward-follow-up-fork-implement-now-vs-later). **Cadence:** Monday = full routine; Tue–Fri = lighter; Sunday ≈ 10 min week-ahead (FEC, registration, brief registry); Friday adds a two-line post-mortem — see [menu-reference.md](skill-work/work-coffee/menu-reference.md) § Cadence by weekday. **Signing-off `coffee`** (session end / wrapping — **same** trigger): **Step 1** = `operator_handoff_check.py` (or `operator_coffee.py --mode closeout`) + summary, then the **same** hub as work-start; **B — Steward** without gate/template split → **system pick** — see [menu-reference — signing-off intent](skill-work/work-coffee/menu-reference.md#signing-off-intent).
...
- **Coffee:** After **Step 1**, fixed **A–D** hub (**no** micro-hints row) — **A — Build**, **B — Steward** (gate / template / integrity / git/ship; **reconciliation code** when template/boundary is in scope per [work-companion-self/README — Reconciliation code audit](skill-work/work-companion-self/README.md#reconciliation-code-audit-upstream-and-downstream)), **C — Strategy (daily brief)** (brief + §1d Putin + §1e JD Vance + optional KY-4; survey defaults here with **`coffee survey`**), **D — (system choice)**. **Conductors** are **out of band** — [conductor skill](../../.cursor/skills/conductor/SKILL.md). **Signing-off:** **B** without steward sub-track → **system pick**. **C** exits to normal workflow unless **`stay in coffee`**; **A**, **B**, **D** re-offer the hub by default; **B** per steward fork. See `.cursor/skills/coffee/SKILL.md` and [menu-reference.md](skill-work/work-coffee/menu-reference.md).
