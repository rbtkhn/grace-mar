...
**What it is:** After a bot conversation, the user may want to review what happened. TELEGRAM-ARCHIVE.md has the raw log, but there's no summary, no "what's new in PENDING," no diff of profile changes. User must read archive + PENDING-REVIEW manually.
