...
| Dimension | work-dev | work-politics (WAP) | Aligned? |
|-----------|----------|----------------------------------|-----------|
...
**Verdict:** Structure is aligned at the “README + workspace + principles + business surface” level. WAP adds gate-sync policy, lanes, and weekly rhythm; work-dev adds integration-status, known-gaps, provenance-checklist. Differences are domain-appropriate.

---

## 3. RECURSION-GATE usage

| Aspect | work-dev | WAP | Aligned? |
|--------|----------|-----|----------|
| **Territory tag** | No dedicated territory string. Handback candidates use `channel_key` (e.g. `openclaw:stage`, `operator:cli`). Treated as **companion** in `recursion_gate_territory.py`. | Explicit **`territory: work-politics`** (and legacy `work-american-politics`); **`channel_key: operator:wap`** or `operator:wap:*`. Filtered as WAP in `recursion_gate_territory.py`. | ✅ By design: work-dev = default companion pipeline; WAP = territory-filtered batch merge. |
| **Batch merge** | `process_approved_candidates.py` default (no `--territory`) = all approved. | `--territory wap` → only WAP rows merged; `--territory companion` → only non-WAP. | ✅ Same script; WAP uses territory to separate batches. |
| **Doc-only vs stage** | Not formalized in README; staging is per handback/session. | Explicit: “Doc-only (no candidate)” vs “Stage to RECURSION-GATE when” (Voice/PRP, paid/milestone audit, explicit companion approval). Rhythm: weekly WAP candidate or “doc-only this week.” | ⚠️ work-dev could add a short “when to stage” (e.g. after OpenClaw session, before export refresh) for parity. |
...
**Verdict:** Gate usage is aligned: one gate file, same merge script; WAP has territory and rhythm for batch separation; work-dev correctly stays “companion” and does not need a territory tag unless you introduce one later.

---

## 4. Operator surface and tooling

| Surface | work-dev | WAP |
|---------|----------|-----|
...
| **Harness warmup** | `harness_warmup.py`; `--territory wap` = WAP pending only; `companion` = Record pending only. | Same; WAP uses warmup for “pending WAP gate items.” |
| **Lanes** | No `docs/lanes/work-dev.md`. | **docs/lanes/work-politics.md** — north star, done looks like, weekly minimum. |
| **Bootstrap** | Default session focus in `grace-mar-bootstrap.md` is work-dev (README + openclaw-integration). | WAP is “other” focus; operator finds WAP via lanes and workspace. |

**Verdict:** work-dev is the default bootstrap focus; WAP has a dedicated lane and weekly rhythm. No conflict; optional improvement: add `docs/lanes/work-dev.md` for parity if you want a “north star” for dev territory.

---

## 5. Cross-references

| From → To | work-dev → WAP | WAP → work-dev |
|-----------|----------------|----------------|
| **README** | No reference to WAP. | No reference to work-dev. Sentient-framing and simple-in-long-term list “build-ai” (legacy name) among territories (politics, gems, health, build-ai). |
...
- **WAP** still says “build-ai” in sentient-framing and simple-in-long-term — should say **work-dev** (or “dev”) for consistency.
- **work-dev** does not mention WAP or other territories; optional to add one line (e.g. “Other work territories: work-politics, work-civ-mem; same gate, different territory tags.”).

**Verdict:** Cross-refs are mostly one-way to shared docs. Naming fix: WAP docs that list territories should use “work-dev” instead of “build-ai.”
...
WAP also has: outreach-workspace, outreach-funnel, consulting-charter, compliance-checklist, revenue-log.
...
|----------|--------|--------|
| **High** | In WAP: replace “build-ai” with “work-dev” in sentient-framing.md and simple-in-long-term-speculation.md where territories are listed. | Docs |
...
| **Low** | Consider adding `docs/lanes/work-dev.md` (north star, done looks like, “never without approval”) if you want lane parity with WAP. | work-dev |
...
- **Structure:** Aligned at README/workspace/principles level; WAP adds gate-sync and lanes; work-dev adds integration-status and provenance.
- **RECURSION-GATE:** Correctly split: work-dev = companion (no territory); WAP = territory + batch merge + rhythm.
- **Naming:** WAP still references “build-ai”; should use “work-dev.”
