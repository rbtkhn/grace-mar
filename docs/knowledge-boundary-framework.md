...
| **5** | **Pre-change harness gate** | Require running `scripts/run_counterfactual_harness.py` before any change to `bot/prompt.py` or `scripts/counterfactual_pack/probes.json`. Option A: document in contributing.md or OPERATOR-WEEKLY-REVIEW. Option B: add a CI job or pre-commit hook that runs the harness when those files change and fails on regressions. | contributing.md / CI / or ops doc update |
