...
|----|------------------------------|----------|
| **D1** | `D1. Conductor — <Name> — continue last emphasis` | **Menu line must include** exactly **one** conductor name (Title Case; see [.cursor/skills/coffee/SKILL.md](../../../.cursor/skills/coffee/SKILL.md) Step 2). Reuse the **same movement emphasis** as the **last** completed Conductor run recorded on disk (see **Persistence**). If **none** is found, use **no logged stack** in place of `<Name>`, say so in one line when they pick **D1**, and use **D2** logic or ask for **one** named movement. |
| **D2** | `D2. Conductor — <Name> — system recommended` | **Menu line must include** exactly **one** system-recommended conductor. Infer **one** movement from **operational** signals in **priority order** — not Record, not policy. **Resolution order** (first match wins): (1) `users/<id>/last-dream.json` — risky worktree / `worktreeAdvice` → **Toscanini**; `tomorrow_inherits` or steward-flavored hints → **Kleiber**; (2) collapsed **Last dream** / **Dream → coffee menu** in Step 1 — lean **C — Strategy** → **Bernstein**; lean **B — Steward** → **Kleiber**; lean **A — Build** implication → **Toscanini**; (3) `Session load: … (recommended: … )` from `operator_coffee.py` — **A** → **Toscanini**, **B** → **Kleiber**, **C** → **Bernstein**; (4) if still empty, **Furtwängler**. **Heuristic** — the operator may override. *(The offline 15–45m ritual may still sequence multiple movements; the **coffee** Step 2 lines stay one name each.)* |

**On pick D1 or D2,** give the usual **short** Conductor orientation ([COFFEE-CADENCE-CONDUCTOR-PROTOCOL.md](../work-strategy/strategy-notebook/COFFEE-CADENCE-CONDUCTOR-PROTOCOL.md) + **concrete** next actions for the **chosen** movement.

**Persistence (D1):** Look for the **most recent** `coffee_pick` in [work-cadence/work-cadence-events.md](work-cadence/work-cadence-events.md) for this user with **`picked=D1`** or **`picked=D2`** and a `conductor=` value (legacy: **`picked=D`**). **Convention:** `conductor=` is a **single** slug: `toscanini` \| `furtwangler` \| `bernstein` \| `karajan` \| `kleiber`. **Legacy** lines may contain `a+b`; for the **D1** menu label use the **first** slug only. **New** logs must **not** use `+`. If **no** line exists, D1 is **empty**. **Optional** log after the turn: `python3 scripts/log_cadence_event.py --kind coffee_pick -u grace-mar --ok --kv picked=D1 conductor=kleiber` (or `picked=D2 conductor=...`) — [coffee SKILL — Cadence audit](../../../.cursor/skills/coffee/SKILL.md#cadence-audit).
