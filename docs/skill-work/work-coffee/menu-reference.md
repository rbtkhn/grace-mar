...
**Historian C — three-option submenu:** Choosing **C — Historian** presents exactly three actionable options and nothing else:
...
**C-B Bookshelf quiz:** [.cursor/skills/bookshelf-knowledge-mcq-to-gate/SKILL.md](../../../.cursor/skills/bookshelf-knowledge-mcq-to-gate/SKILL.md) — default **6** topic-anchored MCQs, **≤2** date-primary stems, strictness default **top2**, staged candidates only; no merge without companion approval.
...
**Capitalist D — bookshelf product use:** D may use bookshelf for offers, teaching, grace-gems, product packaging, or public copy. It does **not** own IX-A bookshelf recursion. Catalog stance membrane remains a scripted secondary path when requested or when a pressing organizational membrane issue applies.
...
The sections below retain **legacy A–G workload detail** (seven letters) for **scope** descriptions; map to the coffee SKILL’s current **A–E** hub (**Daily Brief → C-A Intel**, **Bookshelf self-knowledge quiz → C-B**, **History Notebook / Predictive History → C-C**, **Build → B Engineer**, **Steward → A**, **Symphony continuation → hub E** or standalone **`conductor`**; commercial / teaching bookshelf uses → **D Capitalist**). **Signing-off** add-ons, cadence tables, explicit phrase modifiers, and companion survey track follow.
...
**Canonical Step 2** today: [.cursor/skills/coffee/SKILL.md](../../../.cursor/skills/coffee/SKILL.md) — **A–E** (hub only). **Standalone Conductor** / masters MCQ — [CONDUCTOR-PASS.md](CONDUCTOR-PASS.md). The **rows below** keep **historical seven-letter labels** for workload scope. **Map:** old **Daily Brief → current C-A Intel**; old **Bookshelf self-knowledge quiz → current C-B**; old **Book / Jiang / Predictive History → current C-C Notebook synthesis**; old **Build → current B**; old **Steward → current A**; commercial bookshelf uses → **D**; **Conductor continuation → hub E**.
...
| **E** | **Steward / System choice** | Legacy self-knowledge quiz detail now maps to **current C-B Bookshelf quiz**. Steward tracks remain under current **A — Steward**; Conductor remains current **E — Conductor**. **Read-only membrane invariant:** no merge without companion `approve` + `process_approved_candidates.py`. |
...
### Historian C — Intel, Bookshelf Quiz, Notebook

**Operator intent (grace-mar default):** **`coffee` → C — Historian** does **not** auto-run daily brief and does **not** auto-offer Tri-Frame. It presents exactly three actionable options:

- **A. Intel** — daily brief file + §1d / §1e / §1g / §1h (PRC / IRI when load-bearing) + optional KY-4 intel + optional `last30days` frontier check.
- **B. Bookshelf quiz** — self-knowledge MCQs toward IX-A candidates; default 6 questions, ≤2 date-primary, top2 strictness, staged candidates only.
- **C. Notebook synthesis** — History Notebook / Predictive History synthesis. Tri-Frame minds **Barnes → Mearsheimer → Mercouris** live here; use [daily-brief-minds-menu.md](../work-strategy/daily-brief-minds-menu.md) and [minds/DAILY-BRIEF-MINDS-WORKFLOW.md](../work-strategy/minds/DAILY-BRIEF-MINDS-WORKFLOW.md) when a synthesis pass needs those lenses.
