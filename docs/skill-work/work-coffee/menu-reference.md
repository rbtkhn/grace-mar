...
|--------|------|---------------------------|
| **A** | **Daily Brief** | **Daily operational picture — only when A is chosen** (Step 1 never runs the brief generator or KY-4 web intel). **Default two-stage flow:** **(A)** Brief file + watches + optional KY-4 intel (below). **(B)** In the **same assistant turn**, present the **three Tri-Frame mind lenses** — **Barnes → Mearsheimer → Mercouris** — and **wait for the operator to choose which mind**; then run that lens via [daily-brief-minds-menu.md](../work-strategy/daily-brief-minds-menu.md) (**A–D** per mind). **Do not** stop after Polymarket/polls alone unless the operator opted out (`intel only`, `no tri-frame`, or survey-only). **(1) Daily brief:** When today's `docs/skill-work/work-strategy/daily-brief-YYYY-MM-DD.md` is missing, or the operator asks for refresh: `python3 scripts/generate_work_politics_daily_brief.py -u grace-mar -o docs/skill-work/work-strategy/daily-brief-$(date +%Y-%m-%d).md`. **Monday / full** cadence → run generator on **A**; **Tue–Fri** → generate if missing, else summarize from disk unless refresh requested. **(2) Putin — last 48 hours:** per [daily-brief-putin-watch.md](../work-strategy/daily-brief-putin-watch.md) — web scan; **update §1d** (`## 1d. Putin — last 48 hours`) **in the daily brief file** (edit markdown after generation). **(3) JD Vance — last 48 hours:** per [daily-brief-jd-vance-watch.md](../work-strategy/daily-brief-jd-vance-watch.md) — web scan; **update §1e** (`## 1e. JD Vance — last 48 hours`) **in the daily brief file** (edit markdown after generation). **(4) Optional work-politics intel (KY-4):** run **Polymarket + poll search + Massie X** per [polling-and-markets.md](../work-politics/polling-and-markets.md) and `.cursor/skills/politics-massie/SKILL.md` (weight per cadence) when the operator wants that slice **unless** this turn is **survey-only** (`coffee survey` + operator wants survey without markets — honor that). **(5)** Brief registry, campaign, queue — **one** next step. **Ship:** committing the brief is operator lane. **Not** template-only parity ( **E** ); **not** Rome-only depth ( **C** ). |
