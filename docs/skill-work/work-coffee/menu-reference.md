...
**Canonical Step 2 menu (grace-mar):** **nine lines** — **A** Build, **B** Steward, **C** Strategy (daily brief), **D1** Toscanini, **D2** Furtwängler, **D3** Bernstein, **D4** Karajan, **D5** Kleiber, **E** (system choice). The five conductor lines are **explicit** and **stable** on the menu; continuity and recommendation cues may be mentioned in prose, but they do not own the menu letters. **No** micro-hints line under the menu. Executable contract: [.cursor/skills/coffee/SKILL.md](../../../.cursor/skills/coffee/SKILL.md).

**D1–D5 — Conductor** are **only** the **strategy-notebook** cadence in the menu: operator-as-conductor for the “Symphony of Civilization” per [COFFEE-CADENCE-CONDUCTOR-PROTOCOL.md](../work-strategy/strategy-notebook/COFFEE-CADENCE-CONDUCTOR-PROTOCOL.md). The **shared five-movement model** (usable **by analogy** in work-dev, work-politics, work-cici, etc.) lives in [CONDUCTOR-PASS.md](CONDUCTOR-PASS.md) — it does **not** rename other lanes as **D1–D5**. **No submenu:** D1–D5 are **first-class** lines on the same Step 2 paste. Full spec: [CONDUCTOR-PASS.md — D1–D5](CONDUCTOR-PASS.md#conductor-d1-d5). **self-skill-write** / PRP / Lexile / prompt proposals → **E** (system choice) or explicit **`write`** / **`skill-write`** without opening **`coffee`**.

<a id="conductor-fork-d1-d5"></a>

The sections below retain **legacy A–G workload detail** (seven letters) for **scope** descriptions; map to the coffee SKILL’s current menu as in the spec (**Daily Brief → C**, **Build → A**, **Steward → B**, **Conductor → D1–D5**, **Compass/Book/F/G** → usually **E** or folded into **C**; **old** legacy **D Book** (Jiang) → **E** or **`work-jiang`**, not current **D1–D5**). **Signing-off** add-ons, cadence tables, explicit phrase modifiers, and companion survey track follow.

**Exit:** There is **no** “close hub” letter. The operator leaves coffee by **C** or **any of D1–D5** (normal workflow unless **`stay in coffee`**), by **Later** on the [steward follow-up fork](#steward-follow-up-fork-implement-now-vs-later) (which returns to the full menu), or by starting a non-coffee task. The coffee ritual does **not** use a **no menu** opt-out.

### Bare **`compass`** vs **`coffee`** then **`C`**

**Full ritual:** Say **`coffee`** (Step 1 scripts + full menu), then choose **`C`** when you want **both** re-grounding (warmup, gate snapshot, branch line) **and** the Strategy / daily-brief lane (includes field intel + Tri-Frame minds per skill). **Bare `compass`** (or strategy without opening **`coffee`**) means **strategy lane only**—agents may deliver work-strategy / **ROME-PASS** content **without** re-running Step 1 unless you ask for **`coffee`** first, **`stay in coffee`**, or a cold-thread stack (**`operator_coffee.py --mode reentry`**).
...
- Still output the **full Step 2** menu. On later turns, **A**, **B**, or **E** re-offer the full menu; **B** uses the [steward follow-up fork](#steward-follow-up-fork-implement-now-vs-later); **C** and **D1–D5** exit to normal workflow by default after the reply. **Build (A):** **minimal** — **one** work-dev or skills next step; **git/ship** → **`B git`** if needed; no unrelated sweeps.
...
After **`B — Steward`** (legacy tables below may still say **`E — Steward`**), the assistant **does not** always return to the full menu.
...
**If not actionable** (gate empty for pending; template reconciliation only expected drift / **none** upstream & downstream per exception above; **integrity** clean with no remediation; **git/ship** clean or “no prescription”) → **skip the fork**; re-offer the **full menu** as after **A** / **B** / **E**.
...
**Step 2:** The **same** menu as work-start (**order A, B, C, D1, D2, D3, D4, D5, E**). There is **no** separate closeout menu and **no** closeout-only letter.
...
| **D1–D5** | Optional conductor carryover line grounded in notebook residue from Step 1; keep the add-on aligned with the chosen conductor's emphasis rather than switching lanes. |
