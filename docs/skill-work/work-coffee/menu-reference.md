...
**Canonical Step 2 menu (grace-mar):** **six lines A–D2–E** — **A** Build, **B** Steward, **C** Strategy (daily brief), **D1** Conductor **with one conductor name** — continue last emphasis, **D2** Conductor **with one conductor name** — system recommended, **E** (system choice). **D1** / **D2** each embed a **single** **Title Case** movement name (Toscanini, Furtwängler, Bernstein, Karajan, Kleiber) — no combined `+` stacks on the menu — resolved per [CONDUCTOR-PASS.md](CONDUCTOR-PASS.md#conductor-d1-d2). **No** micro-hints line under the menu. Executable contract: [.cursor/skills/coffee/SKILL.md](../../../.cursor/skills/coffee/SKILL.md).

**D1 / D2 — Conductor** are **only** the **strategy-notebook** cadence in the menu: operator-as-conductor for the “Symphony of Civilization” per [COFFEE-CADENCE-CONDUCTOR-PROTOCOL.md](../work-strategy/strategy-notebook/COFFEE-CADENCE-CONDUCTOR-PROTOCOL.md). The **shared five-movement model** (usable **by analogy** in work-dev, work-politics, work-cici, etc.) lives in [CONDUCTOR-PASS.md](CONDUCTOR-PASS.md) — it does **not** rename other lanes as **D1/D2**. **No submenu:** D1 and D2 are **first-class** lines on the same Step 2 paste. Full spec: [CONDUCTOR-PASS.md — D1 and D2](CONDUCTOR-PASS.md#conductor-d1-d2). **self-skill-write** / PRP / Lexile / prompt proposals → **E** (system choice) or explicit **`write`** / **`skill-write`** without opening **`coffee`**.

<a id="conductor-fork-d1-d2"></a>

The sections below retain **legacy A–G workload detail** (seven letters) for **scope** descriptions; map to the coffee SKILL’s **A–D2–E** as in the spec (**Daily Brief → C**, **Build → A**, **Steward → B**, **Conductor → D1 / D2**, **Compass/Book/F/G** → usually **E** or folded into **C**; **old** legacy **D Book** (Jiang) → **E** or **`work-jiang`**, not current **D1/D2**). **Signing-off** add-ons, cadence tables, explicit phrase modifiers, and companion survey track follow.

**Exit:** There is **no** “close hub” letter. The operator leaves coffee by **C** / **D1** / **D2** (normal workflow unless **`stay in coffee`**), by **Later** on the [steward follow-up fork](#steward-follow-up-fork-implement-now-vs-later) (which returns to the full **A–D2–E** menu), or by starting a non-coffee task. The coffee ritual does **not** use a **no menu** opt-out.

### Bare **`compass`** vs **`coffee`** then **`C`**

**Full ritual:** Say **`coffee`** (Step 1 scripts + **A–D2–E** menu), then choose **`C`** when you want **both** re-grounding (warmup, gate snapshot, branch line) **and** the Strategy / daily-brief lane (includes field intel + Tri-Frame minds per skill). **Bare `compass`** (or strategy without opening **`coffee`**) means **strategy lane only**—agents may deliver work-strategy / **ROME-PASS** content **without** re-running Step 1 unless you ask for **`coffee`** first, **`stay in coffee`**, or a cold-thread stack (**`operator_coffee.py --mode reentry`**).
...
If the operator says **`coffee`** on a **Sunday** (or legacy **`hey`**), default to **week-ahead** mode unless they ask for the full Monday stack. Still run **Step 1** scaled to that mode, then **Step 2** with the full **A–D2–E** menu (labels can be shorter; meanings unchanged).
...
- Still output **full Step 2** menu **A, B, C, D1, D2, E**. On later turns, **A**, **B**, or **E** re-offer the full menu; **B** uses the [steward follow-up fork](#steward-follow-up-fork-implement-now-vs-later); **C**, **D1**, and **D2** exit to normal workflow by default after the reply. **Build (A):** **minimal** — **one** work-dev or skills next step; **git/ship** → **`B git`** if needed; no unrelated sweeps.
...
## Legacy A–G workload table (detail; canonical menu is A–D2–E)

**Canonical Step 2** today: [.cursor/skills/coffee/SKILL.md](../../../.cursor/skills/coffee/SKILL.md) — **six lines A–D2–E**. The **rows below** keep **historical seven-letter labels** for workload scope. **Map:** old **Daily Brief → current C**; old **Build → current A**; old **Steward → current B**; old **Compass / F / G → current E** (system choice) or fold into **C**; old **Book (legacy D) →** **`work-jiang` / E**, **not** current **D1 / D2 — Conductor** (see above).

The **first** coffee reply ends **Step 2** with the fixed menu (present: **A–D2–E**). **Follow-up behavior (canonical letters):** **A**, **B**, and **E** re-offer the full menu by default; **B** uses the [steward follow-up fork](#steward-follow-up-fork-implement-now-vs-later) (full menu only after **Later** or when nothing actionable surfaced); **C**, **D1**, and **D2** exit to normal workflow by default after the reply. Wording may vary; **roles must not**.
...
| **F** | **Cici next** | **work-cici — one next task** when chosen. Ground in Step 1 **`lane next hints`** from `scripts/coffee_lane_next_hints.py` (also runnable alone). Canonical docs: [INDEX.md](../work-cici/INDEX.md), [SYNC-DAILY.md](../work-cici/SYNC-DAILY.md), [WORK-LEDGER.md](../work-cici/WORK-LEDGER.md), [DAILY-OPS-CARD.md](../work-cici/DAILY-OPS-CARD.md). Deliver **one** prescribed step; expand mirrors / BrewMind / runbooks only if needed for that step. In **canonical A–D2–E**, this workload usually maps to **E** (system choice). **Re-offer** full **A–D2–E** after the turn unless the operator exits. |
| **G** | **Dev next** | **work-dev — next task only** from [workspace.md](../work-dev/workspace.md) § **Next actions**: the first **open** numbered line (not leading with `~~`). **One** concrete step — **no** default piggyback of a full legacy **B** (Build / work-dev+skills) or **E git/ship** pass. In **canonical A–D2–E**, this workload usually maps to **E** (system choice). **Re-offer** full **A–D2–E** after the turn unless the operator exits. |

<a id="tri-frame-daily-brief"></a>

### Daily Brief — Tri-Frame minds (three lenses; default second stage)

**Operator intent (grace-mar default):** **`coffee` → C — Strategy (daily brief)** runs **(1)** the daily brief file + §1d / §1e / §1g / §1h (PRC / IRI when load-bearing) + optional KY-4 intel, **then (2)** the assistant **always** offers the **three mind lenses** — **Barnes → Mearsheimer → Mercouris** — and **waits for which mind** to use; the operator then picks **A–D** for that mind from [daily-brief-minds-menu.md](../work-strategy/daily-brief-minds-menu.md). Completing the markdown file alone is **not** the end of **C** unless the operator opted out (**`intel only`**, **`no tri-frame`**, or survey-only).
...
After **`B — Steward`** (legacy tables below may still say **`E — Steward`**), the assistant **does not** always return to the full **A–D2–E** menu.
...
2. **Later** — Immediately present the **full A–D2–E** coffee menu again.

**If not actionable** (gate empty for pending; template reconciliation only expected drift / **none** upstream & downstream per exception above; **integrity** clean with no remediation; **git/ship** clean or “no prescription”) → **skip the fork**; re-offer **full A–D2–E** as after **A** / **B** / **E**.
...
**Step 2:** The **same** menu as work-start (**order A, B, C, D1, D2, E**). There is **no** separate closeout menu and **no** closeout-only letter.
...
| **F** | Cici next step; tie to **lane next hints** + mirror sync / ops card if handoff mentioned exports. |
