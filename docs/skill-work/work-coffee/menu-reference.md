...
The sections below retain **legacy A–G workload detail** (seven letters) for **scope** descriptions; map to the coffee SKILL’s current **A–E** hub (**Daily Brief → C Historian**, **Build → B Engineer**, **Steward → A**, **Symphony continuation → hub E** or standalone **`conductor`**; **Compass/Book/F/G** → **D Capitalist** / **C** as appropriate; Jiang → **`work-jiang`** or **C**). **Signing-off** add-ons, cadence tables, explicit phrase modifiers, and companion survey track follow.

**Exit:** There is **no** “close hub” letter. The operator leaves coffee by **C** (normal workflow unless **`stay in coffee`**), by **Later** on the [steward follow-up fork](#steward-follow-up-fork-implement-now-vs-later) (which returns to the full menu), or by starting a non-coffee task. **A**, **B**, **D**, and **E** re-offer the full **coffee** hub **A–E** **after** the branch completes — **unless hub E** opened **Conductor action MCQ** in **that same reply**, in which case **wait** for **Conductor** **reply A–E** **first** (no duplicate **coffee** hub menu in that message). The coffee ritual does **not** use a **no menu** opt-out.

### Bare **`compass`** vs **`coffee`** then **`C`**

**Full ritual:** Say **`coffee`** (Step 1 scripts + full menu), then choose **`C`** when you want **both** re-grounding (warmup, gate snapshot, branch line) **and** the Historian / daily-brief lane (includes field intel + Tri-Frame minds per skill). **Bare `compass`** (or strategy without opening **`coffee`**) means **strategy lane only**—agents may deliver work-strategy / **ROME-PASS** content **without** re-running Step 1 unless you ask for **`coffee`** first, **`stay in coffee`**, or a cold-thread stack (**`operator_coffee.py --mode reentry`**).
...
|-----|------|-------------|
| **Monday** | **Full** | Complete coffee flow: operator + harness + branch snapshot in Step 1. **Internet intel** (Polymarket, independent polls, Massie X): **not** Step 1 — run on **menu C — Historian** (full Monday weight) or explicit request. **Daily brief:** **menu C** only (Historian / daily brief path). |
| **Tuesday–Friday** | **Lighter** | Same Step 1 as Monday (scripts + branch snapshot). **Polling + Polymarket + Massie X:** only when **C / Historian** (compact) or explicit request. **Daily brief:** **menu C** only (Step 1 one-lines the path pattern). |
| **Sunday** | **Week ahead (~10 min)** | Lighter coffee (week-ahead focus, not Monday-full). Focus: **FEC / compliance dates** and **voter registration** — use **last on-disk** `daily-brief-*.md` calendar slice if present, else [brief-source-registry.md](../work-politics/brief-source-registry.md) (`needs_refresh`, `watch`). **Generating** today's brief remains **menu C — Historian**. Optional: run `operator_work_politics_pulse.py` or check weekly-brief readiness. |
...
- In the **Step 1 warmup brief**, add a short **Companion survey** block (2–4 lines): purpose (IX-B / IX-C refinement), suggested cadence hint (e.g. **monthly micro** 3–5 questions vs **quarterly** deeper pass), pointer that execution is **menu C — Historian** this session unless they choose another letter first.
...
**Canonical Step 2** today: [.cursor/skills/coffee/SKILL.md](../../../.cursor/skills/coffee/SKILL.md) — **A–E** (hub only). **Standalone Conductor** / masters MCQ — [CONDUCTOR-PASS.md](CONDUCTOR-PASS.md). The **rows below** keep **historical seven-letter labels** for workload scope. **Map:** old **Daily Brief → current C**; old **Build → current B**; old **Steward → current A**; old **Compass / F / G → current D** or fold into **C**; old **Book (legacy Jiang D) →** **`work-jiang` / C**; **Conductor continuation → hub E**.

The **first** coffee reply ends **Step 2** with the fixed menu. **Follow-up behavior (canonical letters):** **A**, **B**, **D**, and **E** re-offer the full menu once the branch settles (**exception:** choosing **coffee** hub **`E`** yielded **orientation + Conductor action MCQ** — **do not** re-offer **`coffee`** hub **`A–E`** in **that same reply**; reply to **Conductor** **`A.–E.` first** — see **[Exit]** above · [coffee SKILL — Exit / re-offer](../../../.cursor/skills/coffee/SKILL.md)); **A** uses the [steward follow-up fork](#steward-follow-up-fork-implement-now-vs-later) (full menu only after **Later** or when nothing actionable surfaced); **C** exits to normal workflow by default after the reply. Wording may vary; **roles must not**.

| Letter | Mode | What it means when chosen |
|--------|------|---------------------------|
| **A** | **Daily Brief** | **Daily operational picture — only when A is chosen** (Step 1 never runs the brief generator or KY-4 web intel). **Default two-stage flow:** **(A)** Brief file + watches + optional KY-4 intel (below). **(B)** In the **same assistant turn**, present the **three Tri-Frame mind lenses** — **Barnes → Mearsheimer → Mercouris** — and **wait for the operator to choose which mind**; then run that lens via [daily-brief-minds-menu.md](../work-strategy/daily-brief-minds-menu.md) (**A–D** per mind). **Do not** stop after Polymarket/polls alone unless the operator opted out (`intel only`, `no tri-frame`, or survey-only). **(1) Daily brief:** When today's `docs/skill-work/work-strategy/daily-brief-YYYY-MM-DD.md` is missing, or the operator asks for refresh: `python3 scripts/generate_work_politics_daily_brief.py -u grace-mar -o docs/skill-work/work-strategy/daily-brief-$(date +%Y-%m-%d).md`. **Monday / full** cadence → run generator on **A**; **Tue–Fri** → generate if missing, else summarize from disk unless refresh requested. **(2) Putin — last 48 hours:** per [daily-brief-putin-watch.md](../work-strategy/daily-brief-putin-watch.md) — web scan; **update §1d** (`## 1d. Putin — last 48 hours`) **in the daily brief file** (edit markdown after generation). **(3) JD Vance — last 48 hours:** per [daily-brief-jd-vance-watch.md](../work-strategy/daily-brief-jd-vance-watch.md) — web scan; **update §1e** (`## 1e. JD Vance — last 48 hours`) **in the daily brief file** (edit markdown after generation). **(4) PRC — last 48 hours (when load-bearing):** per [daily-brief-prc-watch.md](../work-strategy/daily-brief-prc-watch.md) — web scan; **update §1g** (`## 1g. PRC — last 48 hours (People’s Republic of China)`) **in the daily brief file** (after **§1f** in generated files; edit markdown after generation). **(5) IRI — last 48 hours (when load-bearing):** per [daily-brief-iran-watch.md](../work-strategy/daily-brief-iran-watch.md) — web scan; **update §1h** (`## 1h. IRI — last 48 hours (Islamic Republic of Iran)`) **in the daily brief file** (after **§1g** in generated files; complements [islamabad-operator-index.md](../work-strategy/islamabad-operator-index.md); edit markdown after generation). **(6) Optional work-politics intel (KY-4):** run **Polymarket + poll search + Massie X** per [polling-and-markets.md](../work-politics/polling-and-markets.md) and `.cursor/skills/politics-massie/SKILL.md` (weight per cadence) when the operator wants that slice **unless** this turn is **survey-only** (`coffee survey` + operator wants survey without markets — honor that). **(7)** Brief registry, campaign, queue — **one** next step. **Ship:** committing the brief is operator lane. **Not** template-only parity ( **E** ); **not** Rome-only depth ( **C** ). |
...
| **E** | **Steward / System choice** | **Two fixed uses:** **(a) Steward tracks** and **(b) Self-knowledge quiz (bookshelf membrane)**. **Self-knowledge quiz** is always valid under `E`: run 5–10 MCQs per round, continue/stop prompt each round, output report + candidate drafts only. **Steward tracks** remain unchanged when chosen: **gate** \| **template/boundary** \| **integrity/exports** \| **git/ship**. **Default when the operator says `E` only (no sub-track):** one best-fit route from context; if governance load is highest use Steward default (gate if pending else template/boundary), otherwise self-knowledge quiz is allowed as the system-chosen branch. **Read-only membrane invariant:** no merge without companion `approve` + `process_approved_candidates.py`. |
| **F** | **Cici next** | **work-cici — one next task** when chosen. Ground in Step 1 **`lane next hints`** from `scripts/coffee_lane_next_hints.py` (also runnable alone). Canonical docs: [INDEX.md](../work-cici/INDEX.md), [SYNC-DAILY.md](../work-cici/SYNC-DAILY.md), [WORK-LEDGER.md](../work-cici/WORK-LEDGER.md), [DAILY-OPS-CARD.md](../work-cici/DAILY-OPS-CARD.md). Deliver **one** prescribed step; expand mirrors / BrewMind / runbooks only if needed for that step. In the canonical menu, this workload usually maps to **E** (system choice). **Re-offer** the full menu after the turn unless the operator exits. |
...
**If not actionable** (gate empty for pending; template reconciliation only expected drift / **none** upstream & downstream per exception above; **integrity** clean with no remediation; **git/ship** clean or “no prescription”) → **skip the fork**; re-offer the **full menu** as after **A** / **B** / **D** / **E**.
...
|--------|---------------------|
| **A** | **Steward** — same **single-track default** as work-start (**gate** if pending candidates, else **template/boundary**); **`A integrity`**, **`A git`**, **`A all`** when explicit. If handoff flagged **dirty tree / branch noise** → prefer **`A git`**; **manifest / derived churn** → **`A integrity`**. Follow-up: [Implement now vs Later](#steward-follow-up-fork-implement-now-vs-later). |
...
**When:** Operator chose **coffee C — Historian** and the pick is survey — or they began with **coffee survey** and then chose **C** (default survey under **Historian**). **Signing-off intent + A** without a sub-track → **system pick** may include survey as the one recommendation.
