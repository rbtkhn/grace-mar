...
| **F** | **Xavier next** | **work-xavier — one next task** when chosen. Ground in Step 1 **`lane next hints`** from `scripts/coffee_lane_next_hints.py` (also runnable alone). Canonical docs: [INDEX.md](../work-xavier/INDEX.md), [SYNC-DAILY.md](../work-xavier/SYNC-DAILY.md), [WORK-LEDGER.md](../work-xavier/WORK-LEDGER.md), [DAILY-OPS-CARD.md](../work-xavier/DAILY-OPS-CARD.md). Deliver **one** prescribed step; expand mirrors / BrewMind / runbooks only if needed for that step. In **canonical A–D2–E**, this workload usually maps to **E** (system choice). **Re-offer** full **A–D2–E** after the turn unless the operator exits. |
