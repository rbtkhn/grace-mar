...
**Micro-hints (one line under the menu):** `Micro-hints: B+skills/meta | F=xavier next | G=dev next (workspace) | E=gate/template/boundary | actionable E → Implement now / Later`
...
**If not actionable** (gate empty for pending; reconciliation truly *none* on both upstream and downstream with no concrete next adopt step) → **skip the fork**; re-offer **full A–G** as after **A** / **B** / **F** / **G**.
