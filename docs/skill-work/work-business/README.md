...
| Role | Description |
|------|-------------|
| **Deep research** | Structured analysis on specific topics: technical whitepapers, funding, roadmaps, competitors, energy/compute chokepoints. |
| **Pipeline alignment** | Analyses can use the same pipeline as [work-strategy](work-strategy/): neutral perceiver summary, energy-chokepoint hook when relevant, three-lens triangulation, Grace-Mar synthesis. Outputs are drafts; human sign-off before any client or public use. |
| **Evidence-linked** | Sources and citations documented; optional ACT- or RECURSION-GATE candidate when the companion wants an audit line or Record-adjacent capability note. |
