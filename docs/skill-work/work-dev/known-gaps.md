...
Resolution:
A derived Markdown table is generated by `scripts/work_dev/render_agent_surfaces_table.py` into `artifacts/work-dev/agent-surfaces/agent-surfaces-table.md` (see agent-sprawl-control-plane). The JSON registry remains authoritative; the table is for operator inspection only.
Do not:
