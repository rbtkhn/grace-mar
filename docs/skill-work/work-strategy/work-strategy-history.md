...
- **Operator demo:** [notes/DEMO-SKILL-STRATEGY-TRANSCRIPTS.md](strategy-notebook/notes/DEMO-SKILL-STRATEGY-TRANSCRIPTS.md) — executable `skill-strategy` calibration (three ingested transcripts + explicit tri-frame); preflight [`scripts/demo_skill_strategy_transcripts_check.sh`](../../scripts/demo_skill_strategy_transcripts_check.sh); README pointer in [strategy-notebook/README.md](strategy-notebook/README.md). Executive summary: [notes/DEMO-SKILL-STRATEGY-EXECUTIVE-REPORT.md](strategy-notebook/notes/DEMO-SKILL-STRATEGY-EXECUTIVE-REPORT.md) (delivery/readiness; live rubric scores when operator completes Phases 1–5).
- **Daily brief:** Standing **§1e** — [daily-brief-jd-vance-watch.md](daily-brief-jd-vance-watch.md) (J.D. Vance, 48h operator fill on coffee **A — Daily Brief**). Generator emits `## 1e. JD Vance — last 48 hours`; weak-signal block renumbered to **§1f**. [daily-brief-config.json](daily-brief-config.json) adds JD Vance tokens to **pol_keyword_phrases** for **W** scoring (no bare `vance` substring — avoids “advance” false positives).
