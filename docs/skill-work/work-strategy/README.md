...
| **[STRATEGY.md](STRATEGY.md)** | WORK-only ledger: CORE / SCHOLAR-style heuristics / **§IV operator strategy log** (additive notes in-file; not CMC `MEM–*` shards); not Record. |
...
**Operator habit:** Starting Cursor with **`coffee`** runs warmup in [.cursor/skills/coffee/SKILL.md](../../.cursor/skills/coffee/SKILL.md) (see bootstrap); legacy **`hey`** still works. **Generating** today's daily brief to `docs/skill-work/work-strategy/daily-brief-YYYY-MM-DD.md` is **coffee menu C** — Step 1 does **not** run the generator.
