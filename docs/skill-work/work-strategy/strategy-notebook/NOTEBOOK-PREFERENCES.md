...
| **Expert thread journal (page-composed)** | **`transcript.md`** = bulk verbatim **SSOT**; **machine layer** = **`thread`** echo; **journal layer** each **`## YYYY-MM`** = **composed from** that month’s **thread-embedded `strategy-page`** set (Chronicle / in-page `>` first), plus **bookend** connective prose (~**30%** skill-write-style glue; ~**70%** expert-anchored page body — norm, not validator). **Not** a long paraphrase of the transcript. **[`docs/skill-write/`](../../../../docs/skill-write/write-operator-preferences.md)** = **Locals / X / PH**; thread month glue may borrow that register for **lede/closer** without turning the whole journal into public copy. List pages: `list_strategy_pages_by_month.py` (optional `--chronicle-snippets`, `--json`). Architecture + [strategy-expert-template.md](strategy-expert-template.md#thread-template) § Thread. |
...
| **Compose skeleton (primary expert)** | **S1–S5** — when the **EOD session** names one **primary** `strategy-expert` (or MC pick), default **Chronicle / Reflection / References / Predictive Outlook weighting** follows § *Weave skeletons (S1–S5)* (heading unchanged for grep); multi-expert analysis stays in the **`strategy-page`** body (and compressed `days.md` when composed there); primary = **filing / spine**, not sole epistemic authority. |
...
| 2026-04-17 | **Knot length on weave:** **`strategy-notebook-knot-*.md`** target **300–1000 words**; page-shape menu stubs scoped to that band; architecture § *Weave command* + *Weave choice*; template + skill-strategy checklist. |
