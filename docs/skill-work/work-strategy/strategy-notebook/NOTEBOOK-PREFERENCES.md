...
| **Expert corpus (`thread`)** | Inbox lines with **`thread:<expert_id>`** → operator **`thread`** rebuilds per-expert **`experts/<expert_id>/thread.md`** machine layer (and transcripts) — **`python3 scripts/strategy_thread.py`** — **not** a **`weave`** (no `days.md` / knots). Architecture § *Thread (terminology)*. |
