...
**Governed by:** [STRATEGY-NOTEBOOK-ARCHITECTURE.md](STRATEGY-NOTEBOOK-ARCHITECTURE.md) — this file **narrows** daily practice; it does not replace architecture for repo-wide defaults (e.g. architecture still describes a **~1000w** consolidated **`days.md`** daily target where no override applies; **`strategy-page`** prose has **no** word ceiling—**typical** digest passes are often **~500–1000w**). **`weave` (operator token) is deprecated** — use **end-of-day strategy session** / **EOD notebook compose**; see architecture § *End-of-day strategy session*.  
...
| **EOD page-shape menu** | In the **EOD strategy session**, assistants present **4–6** labeled options (**page thesis / shape / content emphasis**) **before** writing to disk—see [STRATEGY-NOTEBOOK-ARCHITECTURE.md](STRATEGY-NOTEBOOK-ARCHITECTURE.md) § *EOD compose — page-shape menu* and § *Page design and notebook-use jobs*. Each stub should **name the implied notebook-use job(s)** (same seven vocations as expert tags — `orient`, `negotiate`, `validate`, `authorize`, `stress-test`, `narrate`, `historicize`) in a **thesis-first** line, not as a tag-only menu. Each option should be **scoped** for a realistic **density** (e.g. “thin page + `days.md` carry,” “full in-thread synthesis,” “Links-heavy / Judgment-tight”)—**no** hard word cap, but typical digests are often **~500–1000 words** in Chronicle/Reflection/Foresight. Operator picks a letter, says **`no menu`**, or names the shape explicitly. **Optional — full MCQ stack:** When the operator wants **session type → lanes → promotion → shape → (page action) → `days.md` mode** before any prose, follow [EOD-MCQ-PROTOCOL.md](EOD-MCQ-PROTOCOL.md) (architecture § *EOD MCQ protocol (v1)*). |
| **Compose skeleton (primary expert)** | **S1–S5** — when the **EOD session** names one **primary** `strategy-expert` (or MC pick), default **Chronicle / Reflection / References / Foresight weighting** follows § *Weave skeletons (S1–S5)* (heading unchanged for grep); multi-expert analysis stays in the **`strategy-page`** body (and compressed `days.md` when composed there); primary = **filing / spine**, not sole epistemic authority. |
| **Page length (EOD output)** | **`strategy-page`:** **no** enforced word limit; typical digest-style prose is often **~500–1000 words** in Chronicle/Reflection/Foresight (`wc -w` excludes **`### Appendix`** when present). Router / deferral pages may be **short** when prose is **explicitly** routed to **`days.md`** per architecture. |
...
| Date | Change |
|------|--------|
| 2026-04-21 | **`strategy-page` / expert refined pages:** **No** enforced word ceiling—typical digest still ~**500–1000** words; align architecture, templates, verifier, assemblers, `skill-strategy`. |
