...
**When to open this doc:** You are about to create or review a **`raw-input/`** file and need **type-specific** defaults for YAML **`kind:`**, **`thread:`**, inbox **stub shape**, and refined-page **Verbatim** handling — use **§ Essay**, **§ Transcript**, **§ Social**, or **§ Wire and institutional PDF** below. For directory layout and formal **`pub_date`** rules, see [README.md](README.md) and [refined-page-template.md](../refined-page-template.md).
...
| **`experts/<id>/<id>-page-*.md`** | **Notebook handle** — **`### Verbatim`** (excerpt or condensed from raw), **`### Reflection` / `### Foresight`**, **`### Appendix`**. Not a second verbatim archive; [refined-page-template.md](../refined-page-template.md) § SSOT hierarchy. |
...
| Axis | Guidance |
|------|----------|
| **Typical sources** | Substack, Ghost, Medium, HTML paste, RSS **`fetch_strategy_raw_input`** pulls. |
...
| **Raw body** | Full article text under YAML; preserve headings for navigation; if **paywalled / partial**, state in YAML **`note:`** or inbox (**`partial`**). |
...
| **Refined `### Verbatim`** | Often **exceeds** ~3k word budget — **excerpt** key sections; full text stays in **`raw-input/`**; note omissions in **`### Appendix`** ([refined-page-template.md](../refined-page-template.md) § Length). |
...
| **Refined `### Verbatim`** | Often **lane-specific** (guest-only, or expert monologue) or **head + tail + omission line** per [refined-page-template.md](../refined-page-template.md) § Length; drop host filler if budget forces. |
...
| Axis | Guidance |
|------|----------|
| **Typical sources** | Copy-paste posts; thread exports; Locals HTML paste; **screenshot rolls** with filenames under **`assets/`**. |
| **`kind:`** | Prefer **`x-post-text`** for direct text paste; **`paste-bundle`** for mixed clips; **`mixed`** for hybrid; **`screenshot-list`** / **`x-screenshots-index`** when **no** extractable speech text ([README.md — kinds table](README.md)). |
...
| **Refined `### Verbatim`** | Short posts may **fit full** Verbatim; long threads → **selected posts** + Appendix pointer to raw bundle. |
...
- [refined-page-template.md](../refined-page-template.md) — Verbatim budget, Appendix bullet order
