...
3. `### Reflection` — **Operator analysis:** Sachs-lane arc; keep verification discipline explicit where load-bearing claims sit in commentator tier. **May be stubbed** until a later operator pass.
...
6. `### Appendix` — machinery only (paths, inbox grep, `thread:sachs`, canonical URL).
...
- `Refined day page:` → `sachs-page-YYYY-MM-DD.md` or `sachs-page-YYYY-MM-DD-<slug>.md` (one line per refined file).
...
1. Title: `# Sachs day page — YYYY-MM-DD`. Optional short title in parentheses when multiple pages share the date; filename uses `-<slug>` when split.
