...
```markdown
**Expert:** `diesen` · **Aired / published:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/diesen/`). Not a `strategy-page` HTML fence in `thread.md` unless you duplicate the same judgment there during EOD compose.
```
...
```markdown
**Expert:** `diesen` · **Ingest:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/diesen/`). **Note:** This date may be a `notebook |` lens row only—not a full capture verbatim. [Optional: tri-mind / chapter pointer — e.g. `days.md` § anchor.]
```
...
3. `### Reflection` — **Operator analysis:** Diesen-lane arc; keep verification discipline explicit where load-bearing claims sit in commentator tier. **May be stubbed** until a later operator pass.
...
6. `### Appendix` — machinery only (paths, inbox grep, `thread:diesen`, canonical URL).
...
2. **Inbox / triage:** link to [`daily-strategy-inbox.md`](../../daily-strategy-inbox.md) with grep hints (`thread:diesen`, date, and for Mode B the `notebook |` / `verify:` tail).
3. **`thread:diesen`** · **verify:** one line consolidating verify tier for this page.
...
## Raw-input naming (Diesen)

Verbatim lives under `raw-input/<date>/`. Refined pages live only under `experts/diesen/`.
...
- `Refined day page:` → `diesen-page-YYYY-MM-DD.md` or `diesen-page-YYYY-MM-DD-<slug>.md` (one line per refined file).
- `Template:` → [`diesen-page-template.md`](diesen-page-template.md) (optional header pointer).
...
- **Inbox / triage:** [daily-strategy-inbox.md](../../daily-strategy-inbox.md) (search `thread:diesen`, YYYY-MM-DD)
- **`thread:diesen`** · **verify:** full-text + raw-input + aired:YYYY-MM-DD
...
1. Title: `# Diesen day page — YYYY-MM-DD`. Optional short title in parentheses when multiple pages share the date; filename uses `-<slug>` when split.
2. `WORK only; not Record.` on its own line after the title.
3. Preamble matches Mode A or Mode B; **Artifact:** uses “refined day page (standalone file under `experts/diesen/`)” verbatim.
4. `---` before `### Chronicle`.
...
6. `---` before `### Appendix`.
7. Appendix bullets in order: Full verbatim (capture) → Inbox / triage → `thread:diesen` · verify → Canonical primary.
8. Relative links resolve from `experts/diesen/` (typically `../../raw-input/…`, `../../daily-strategy-inbox.md`, `../../chapters/…` when citing `days.md`).
