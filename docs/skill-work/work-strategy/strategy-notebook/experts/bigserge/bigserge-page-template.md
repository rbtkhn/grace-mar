...
**Purpose:** Standalone Chronicle / Reflection / Foresight artifact for the `bigserge` expert. Not a substitute for full verbatim in [`raw-input/`](../../raw-input/README.md). Distinct from a `strategy-page` HTML fence in [`thread.md`](thread.md) unless you duplicate judgment there during EOD compose.
...
```markdown
**Expert:** `bigserge` · **Aired / published:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/bigserge/`). Not a `strategy-page` HTML fence in `thread.md` unless you duplicate the same judgment there during EOD compose.
```
...
```markdown
**Expert:** `bigserge` · **Ingest:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/bigserge/`). **Note:** This date may be a `notebook |` lens row only—not a full capture verbatim. [Optional: tri-mind / chapter pointer — e.g. `days.md` § anchor.]
```
...
3. `### Reflection` — Big Serge-lane arc; keep verification discipline explicit where load-bearing claims sit in commentator tier.
4. `### Foresight` — falsifiers, resume lines, tier tags (bullets allowed).
5. Horizontal rule `---` before appendix.
6. `### Appendix` — machinery only (paths, inbox grep, `thread:bigserge`, canonical URL).
...
2. **Inbox / triage:** link to [`daily-strategy-inbox.md`](../../daily-strategy-inbox.md) with grep hints (`thread:bigserge`, date, and for Mode B the `notebook |` / `verify:` tail).
3. **`thread:bigserge`** · **verify:** one line consolidating verify tier for this page.
...
## Raw-input naming (Big Serge)

Verbatim lives under `raw-input/<date>/`. Refined pages live only under `experts/bigserge/`.
...
- **Inbox / triage:** [daily-strategy-inbox.md](../../daily-strategy-inbox.md) (search `thread:bigserge`, YYYY-MM-DD)
- **`thread:bigserge`** · **verify:** full-text + raw-input + aired:YYYY-MM-DD
...
1. Title: `# Big Serge day page — YYYY-MM-DD`.
2. `WORK only; not Record.` on its own line after the title.
3. Preamble matches Mode A or Mode B; **Artifact:** uses “refined day page (standalone file under `experts/bigserge/`)” verbatim.
4. `---` before `### Chronicle`.
...
6. `---` before `### Appendix`.
7. Appendix bullets in order: Full verbatim (capture) → Inbox / triage → `thread:bigserge` · verify → Canonical primary.
8. Relative links resolve from `experts/bigserge/` (typically `../../raw-input/…`, `../../daily-strategy-inbox.md`, `../../chapters/…` when citing `days.md`).
