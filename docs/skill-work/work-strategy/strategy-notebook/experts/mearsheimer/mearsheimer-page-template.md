...
**Purpose:** Standalone Chronicle / Reflection / Foresight artifact for the `mearsheimer` expert. Not a substitute for full verbatim in [`raw-input/`](../../raw-input/README.md). Distinct from a `strategy-page` HTML fence in [`thread.md`](thread.md) unless you duplicate judgment there during EOD compose.
...
```markdown
**Expert:** `mearsheimer` · **Aired / published:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/mearsheimer/`). Not a `strategy-page` HTML fence in `thread.md` unless you duplicate the same judgment there during EOD compose.
```
...
```markdown
**Expert:** `mearsheimer` · **Ingest:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/mearsheimer/`). **Note:** This date may be a `notebook |` lens row only—not a full capture verbatim. [Optional: tri-mind / chapter pointer — e.g. `days.md` § anchor.]
```
...
3. `### Reflection` — Mearsheimer-lane arc; keep verification discipline explicit where load-bearing claims sit in commentator tier.
4. `### Foresight` — falsifiers, resume lines, tier tags (bullets allowed).
5. Horizontal rule `---` before appendix.
6. `### Appendix` — machinery only (paths, inbox grep, `thread:mearsheimer`, canonical URL).
...
2. **Inbox / triage:** link to [`daily-strategy-inbox.md`](../../daily-strategy-inbox.md) with grep hints (`thread:mearsheimer`, date, and for Mode B the `notebook |` / `verify:` tail).
...
Verbatim lives under `raw-input/<date>/`. Refined pages live only under `experts/mearsheimer/`.
...
- `Refined day page:` → `mearsheimer-page-YYYY-MM-DD.md`.
...
- **`thread:mearsheimer`** · **verify:** full-text + raw-input + aired:YYYY-MM-DD
...
1. Title: `# Mearsheimer day page — YYYY-MM-DD`.
2. `WORK only; not Record.` on its own line after the title.
3. Preamble matches Mode A or Mode B; **Artifact:** uses “refined day page (standalone file under `experts/mearsheimer/`)” verbatim.
4. `---` before `### Chronicle`.
