...
```markdown
**Expert:** `parsi` · **Aired / published:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/parsi/`). Not a `strategy-page` HTML fence in `thread.md` unless you duplicate the same judgment there during EOD compose.
```
...
```markdown
**Expert:** `parsi` · **Ingest:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/parsi/`). **Note:** This date may be a `notebook |` lens row only—not a full capture verbatim. [Optional: tri-mind / chapter pointer — e.g. `days.md` § anchor.]
```
...
3. `### Reflection` — **Operator analysis:** Parsi-lane arc; keep verification discipline explicit where load-bearing claims sit in commentator tier. **May be stubbed** until a later operator pass.
...
2. **Inbox / triage:** link to [`daily-strategy-inbox.md`](../../daily-strategy-inbox.md) with grep hints (`thread:parsi`, date, and for Mode B the `notebook |` / `verify:` tail).
3. **`thread:parsi`** · **verify:** one line consolidating verify tier for this page.
...
## Raw-input naming (Parsi)

Verbatim lives under `raw-input/<date>/`. Refined pages live only under `experts/parsi/`.
...
- `Refined day page:` → `parsi-page-YYYY-MM-DD.md` or `parsi-page-YYYY-MM-DD-<slug>.md` (one line per refined file).
- `Template:` → [`parsi-page-template.md`](parsi-page-template.md) (optional header pointer).
...
- **`thread:parsi`** · **verify:** full-text + raw-input + aired:YYYY-MM-DD
...
1. Title: `# Parsi day page — YYYY-MM-DD`. Optional short title in parentheses when multiple pages share the date; filename uses `-<slug>` when split.
