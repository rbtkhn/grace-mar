...
3. `### Reflection` — **Operator analysis:** Johnson-lane arc; keep verification discipline explicit where load-bearing claims sit in commentator tier. **May be stubbed** until a later operator pass.
...
- `Refined day page:` → `johnson-page-YYYY-MM-DD.md` or `johnson-page-YYYY-MM-DD-<slug>.md` (one line per refined file).
...
1. Title: `# Johnson day page — YYYY-MM-DD`. Optional short title in parentheses when multiple pages share the date; filename uses `-<slug>` when split.
