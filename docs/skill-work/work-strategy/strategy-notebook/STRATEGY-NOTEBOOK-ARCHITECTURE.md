...
2. **Compose or revise thread-embedded pages** (`<!-- strategy-page:start` … in `experts/<expert_id>/thread.md` under `## YYYY-MM`) on **explicit** operator weave intent or an **end-of-day page session** (often beside **`dream`**) — **synthesis** (required **Signal / Judgment / Open** prose; **`### Technical appendix`** optional when machinery is needed — see [strategy-page-template.md](strategy-page-template.md)), not an inbox mirror. Update `days.md` continuity entry for the same date.
...
**Rule of thumb:** **Inbox** = intake SSOT for rough capture; **pages** (marker-fenced blocks in expert **`thread.md`** files) = judgment after weave; **`days.md`** = chronology and continuity.

**Default output path (chat / assistant):** chat → inbox → **expert-thread pages + `days.md` continuity only on explicit weave / EOD page session** — same discipline as [Expert choreography](#expert-choreography) *Output path (default)* below.

## Weave (terminology)

**Weave** is the **operator command** to compose or revise **strategy pages** (thread-embedded `strategy-page` blocks) from [daily-strategy-inbox.md](daily-strategy-inbox.md) material and update `days.md` continuity. The verb describes the composing action; the **page** (per expert `thread.md`) is the product. Say **`weave`**, or run **`strategy`** with explicit **weave** intent. Older grep/git and JSONL fields may keep `fold` (e.g. `fold_kind` in [FOLD-LEARNING.md](FOLD-LEARNING.md)). **Note:** Later **“Weaving …”** bullets under § *Daily strategy inbox* (Rome, Putin, §1 watches) mean **integrating a channel over months** — related textile image, **not** the same token as the **`weave`** command.
...
2. **Extraction** — reads each expert's **`transcript.md`** (what the expert said recently) and lists **`strategy-page`** blocks already in **`thread.md`**, writes **machine-maintained** material to **`experts/<expert_id>/thread.md`** **between** the HTML comment markers (see below). **Human narrative** lives **outside** that block.
...
**What it is not:** **`thread`** does **not** update **`days.md`**, journal-layer **`strategy-page`** blocks, or the inbox **`Accumulator for:`** line. It is **not** a substitute for **`weave`**. Transcript or aggregator output still lands in the **inbox** first (paste-ready lines + **`thread:`** when the cold line attributes speech to a named indexed expert).
...
**Dual on-disk layout:** [`rebuild_threads`](../../../../scripts/strategy_expert_corpus.py) (via **`thread`**) writes **`experts/<expert_id>/thread.md`** only. Some trees also keep a flat **`strategy-expert-<expert_id>-thread.md`** at the notebook root; [`validate_strategy_expert_threads.py`](../../../../scripts/validate_strategy_expert_threads.py) accepts either. Treat **one** path as canonical for human edits in your workflow; after **`thread`**, confirm which file’s machine block updated so you do not edit a stale duplicate.
...
| **Strategy-page** | `<!-- strategy-page:start` … `<!-- strategy-page:end -->` in `experts/<expert_id>/thread.md` | **Expert-thread judgment** unit (primary written unit for weave); **many** per month are normal; multi-expert weaves reuse the same **`id=`** across files. |
...
**Going forward:** This chapter / day page / **strategy-page** map is the **default vocabulary** for describing strategy-notebook structure in **new and revised** notebook docs, **skill-strategy** / weave explanations, and assistant answers — unless the sentence is specifically about the **`strategy-page`** HTML fence or tooling. It does **not** require a repo-wide search-replace of legacy “page” wording; apply it when adding or editing relevant material.
...
**Legacy note:** Older text referred to a “knot-shape” menu; the **page-shape** fork above is the **default** assistant contract for **`weave`** unless the operator opts out.

**Weave skeletons (S1–S5):** When the operator picks a **primary `strategy-expert`** for the weave (or accepts a menu default), [NOTEBOOK-PREFERENCES.md](NOTEBOOK-PREFERENCES.md) § *Weave skeletons (S1–S5)* defines **five** optional **spines** (operations, power/incentives, legitimacy/room, domestic machinery, markets/macro) and a **failure mode** per spine — **orthogonal** to the **page-shape menu** above; combine both when useful.
...
**Intra-day weaves** iterate **one** consolidated **`## YYYY-MM-DD`** block in `days.md`: later weaves **merge into** the same heading (edit in place; tighten **Judgment**) unless you need a rare **Update (later weave):** trace; avoid **two parallel essays** for the same calendar day **in that single block**. **`strategy-page` blocks (atomic analysis):** the primary written units live in **`experts/<expert_id>/thread.md`** inside **`## YYYY-MM`** month segments—marker-fenced **`<!-- strategy-page:start … -->`** … **`<!-- strategy-page:end -->`**. **Multiple pages per calendar day** are normal when shapes differ—distinguished by **`id="…"`** on the page marker; **multi-expert** weaves **duplicate** the same logical **`id`** into each involved expert’s thread (see [watches/README.md](watches/README.md)). **Template:** [strategy-page-template.md](strategy-page-template.md). **Page body length (standard weave):** target **500–1000 words** in **Signal / Judgment / Open** (machinery quarantined in **Technical appendix**); if material would exceed **~1000 words**, **route** long quotes, inventories, or alternate theses to **`days.md`**, a second page **`id`**, or **Links** rather than stuffing one page. **Below ~500 words** is allowed for **router / case / link-hub** pages only when the stub explicitly **defers** narrative to `days.md`. **Cadence:** default **end-of-day** page composition—do **not** add or extend **`strategy-page`** blocks continuously through the day unless breaking glass (daytime capture stays in the inbox). The dated `days.md` block is the **chronology and continuity** layer; it should **name** active page **`id`s** and expert lanes so readers can find threads.
