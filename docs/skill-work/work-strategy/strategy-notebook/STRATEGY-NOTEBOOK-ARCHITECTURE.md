...
**Invocation:** Run a **`strategy`** session at day-end with **notebook compose** intent, or describe the same intent in natural language (**“close the strategy day,”** **“EOD notebook,”** **“compose pages from today’s raw-input”**). **`weave` is deprecated** as the operator trigger token — do **not** use it in new assistant menus or docs; keep **`python3 scripts/strategy_weave.py`** as the **read-only** cross-expert analysis tool name ([watches/README.md](watches/README.md)), distinct from notebook **page composition**.
