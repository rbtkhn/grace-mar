...
**`thread`** (operator command — use backticks in prose so it is **not** confused with the inbox verify tail **`thread:<expert_id>`**) means: **rebuild** the per-expert rolling mirrors **`strategy-expert-<expert_id>.md`** from inbox lines that carry **`thread:<expert_id>`**, by running **`python3 scripts/strategy_thread.py`** (delegates to [`scripts/strategy_expert_corpus.py`](../../../../scripts/strategy_expert_corpus.py); same **`--inbox`**, **`--threads`**, **`--out`**, **`--days`**, **`--today`**, **`--dry-run`** flags).

**What it is not:** **`thread`** does **not** update **`days.md`**, knot files, or the inbox **`Accumulator for:`** line. It is **not** a substitute for **`weave`**. Transcript or aggregator output still lands in the **inbox** first (paste-ready lines + **`thread:`** when the cold line attributes speech to a named indexed expert); **`thread`** only **materializes** that stream into the **script-owned** corpus block inside each **`strategy-expert-*.md`**.

**Manual lane:** Edits to the **Seed** section (above the corpus markers) in **`strategy-expert-<expert_id>.md`** are **operator-authored** — do them **outside** the **`thread`** command; the rebuild preserves Seed but **overwrites** the delimited rolling block.
...
**Contextual weave menu (assistant behavior):** When the operator starts a notebook write or **weave**, prefer a **short fork** that depends on context — e.g. if the **daily brief** or **gate** is hot, surface options that match (verify-first / arc / inbox compress); on a **quiet** day, offer synthesis or `meta.md` touch. Same **skill-strategy** / template discipline applies; only the **default menu** shifts.
...
1. **Commentator / expert threads** ([strategy-commentator-threads.md](strategy-commentator-threads.md)) — **longitudinal** lanes: what each named voice said over time so you can track **accuracy**, **narrative drift**, and **compare–contrast** across experts. Ingests use **`thread:<expert_id>`** (see that file). This is the **bookkeeping and evidence** plane for *who said what*. **Rolling mirror:** `strategy-expert-<expert_id>.md` here; rebuild with operator **`thread`** — `python3 scripts/strategy_thread.py` (same engine as `python3 scripts/strategy_expert_corpus.py`) — verbatim inbox lines per slug, script-delimited block vs operator **Seed** (same index file).
...
**Per-expert rolling mirror:** Ingest lines that carry **`thread:<expert_id>`** can be rebuilt into `strategy-expert-<expert_id>.md` via **`python3 scripts/strategy_thread.py`** (operator **`thread`**; same as `python3 scripts/strategy_expert_corpus.py`); crossing rules and optional `verify:` tails stay in [strategy-commentator-threads.md](strategy-commentator-threads.md).
