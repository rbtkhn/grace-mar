...
| **[`raw-input/`](raw-input/README.md)** | **Full retention (7 days)** — **unabridged** transcripts and bundled source material in **`raw-input/YYYY-MM-DD/`** (**Mercouris:** **`mercouris-page-YYYY-MM-DD.md`**; **other experts:** **`YYYY-MM-DD-<expert_id>.md`** for multiple ingests per expert-day; other captures use descriptive slugs). Separate from the inbox. Same rolling calendar window as expert transcripts; **prune is operator-initiated** — **`prune_strategy_raw_input.py --apply`** is **blocked** while **[`raw-input/.pruning-suspended`](raw-input/.pruning-suspended)** exists unless **`--override`** (see [raw-input/README.md](raw-input/README.md) § Pruning). |
