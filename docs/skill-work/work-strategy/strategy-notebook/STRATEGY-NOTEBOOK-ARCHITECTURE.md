...
**Per-expert rolling mirror:** Ingest lines that carry **`thread:<expert_id>`** can be rebuilt into `strategy-expert-<expert_id>.md` via `python3 scripts/strategy_expert_corpus.py`; crossing rules and optional `verify:` tails stay in [strategy-commentator-threads.md](strategy-commentator-threads.md).
