...
- **Fold learning (optional):** [FOLD-LEARNING.md](FOLD-LEARNING.md) — append-only `strategy-fold-events.jsonl`, `log_strategy_fold.py`, `report_strategy_fold_learning.py`
- **Commentator threads (index):** [strategy-commentator-threads.md](strategy-commentator-threads.md) — **`analyst_id`** (first column; inbox: **`thread:<analyst_id>`**) ↔ anchor analyst, **grep** tags, typical **`batch-analysis`** pairings
...
- **Chapters:** `chapters/YYYY-MM/` — `meta.md` (month) + `days.md` (one **`## YYYY-MM-DD` page** per day, append-only) — optional `pages/YYYY-MM-DD.md` per day if you split files
...
- **History notebook (parallel, LIB-0156):** [../history-notebook/README.md](../history-notebook/README.md) — operator mechanism chapters; daily **`### History resonance`** in `days.md` cites chapter ids + warrants (see [STRATEGY-NOTEBOOK-ARCHITECTURE.md](STRATEGY-NOTEBOOK-ARCHITECTURE.md) § *Parallel to History notebook*)
