# Strategy expert — Daniel L. Davis (`daniel-davis`)

WORK only; not Record.

**Canonical index:** [strategy-commentator-threads.md](strategy-commentator-threads.md) — **`daniel-davis`** lane.
...
- `daniel-davis` + **Defense Priorities / restraint-realist** ecosystem — ending endless wars, U.S. interests over global hegemony framing; shared event space with figures like Chas Freeman at DP events.
- `daniel-davis` + **combat-veteran skeptics of escalation** (e.g. Scott Ritter in joint formats) — realistic military assessment vs political rhetoric; **verify** each line’s `thread:` and do not merge ORBAT facts without a seam.
...
- `daniel-davis` × **Pentagon / optimistic official narratives** — 2012 Afghanistan report as precedent: on-ground vs briefing-room story (see NYT 2012; Ridenhour Prize).
- `daniel-davis` × **interventionist hawks** — limits of U.S. power, strategic overreach, NATO/Ukraine or Iran scenarios where escalation is sold as low-cost.
...
- This expert usually tensions when:
  ...
  - **Iran / Hormuz / Taiwan** scenarios need **slide-order** macro distinguished from **wire-grade** ORBAT (pair with `steve-jermy` / `scott-ritter` without collapsing lanes).
...
- Pull this expert into weave when:
  ...
  - You need a **B-plane** that is **not** ORBAT-only (`scott-ritter`) and **not** pure diplomatic “room” (`alexander-mercouris`) — **macro hurt + negotiation clock**.
...
1. https://www.defensepriorities.org/people/daniel-davis/
...
5. https://www.ridenhour.org/recipients/daniel-davis — Ridenhour Prize (corroboration of 2012 reporting arc)
