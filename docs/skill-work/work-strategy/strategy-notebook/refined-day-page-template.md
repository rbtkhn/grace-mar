# Refined day page template (all experts)
...
```markdown
**Expert:** `{expert_id}` · **Published:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/{expert_id}/`). Not a `strategy-page` HTML fence in `thread.md` unless you duplicate the same judgment there during EOD compose.
```
...
```markdown
**Expert:** `{expert_id}` · **Ingest:** YYYY-MM-DD · **Artifact:** refined day page (standalone file under `experts/{expert_id}/`). **Note:** This date may be a `notebook |` lens row only—not a full capture verbatim. [Optional: tri-mind / chapter pointer — e.g. `days.md` § anchor.]
```
...
```markdown
**Expert:** `ritter` · **Published:** YYYY-MM-DD · **Capture:** Mode C — Substack · **Artifact:** refined day page (standalone file under `experts/ritter/`). Not a `strategy-page` HTML fence in `thread.md` unless you duplicate the same judgment there during EOD compose.
```
...
```markdown
**Expert:** `ritter` · **Published:** YYYY-MM-DD · **Capture:** Mode D — Judging Freedom / interview (see raw-input `series`) · **Artifact:** refined day page (standalone file under `experts/ritter/`). Not a `strategy-page` HTML fence in `thread.md` unless you duplicate the same judgment there during EOD compose.
```
...
```markdown
**Expert:** `ritter` · **Published:** YYYY-MM-DD · **Capture:** Mode E — YouTube (see raw-input `source_url`) · **Artifact:** refined day page (standalone file under `experts/ritter/`). Not a `strategy-page` HTML fence in `thread.md` unless you duplicate the same judgment there during EOD compose.
```
...
```markdown
**Expert:** `ritter` · **Ingest:** YYYY-MM-DD · **Capture:** Mode F — notebook / X stub · **Artifact:** refined day page (standalone file under `experts/ritter/`). **Note:** May lack a single long capture—point to `days.md` / chapter seam when the lens hangs off a woven day.
```
...
- **`Refined day page:`** → `{expert_id}-page-YYYY-MM-DD.md` or `{expert_id}-page-YYYY-MM-DD-<slug>.md` (one line per refined file).
- **`Template:`** → optional pointer to this canonical file **or** to the thin **`{expert_id}-page-template.md`** stub in the same folder (compat links).
...
```markdown
# {ExpertTitle} day page — YYYY-MM-DD
...
1. Title: `# … day page — YYYY-MM-DD` (optional short parenthetical when multiple pages share the date); filename uses `-<slug>` when split.
2. `WORK only; not Record.` on its own line after the title.
3. Preamble matches one chosen mode; **Artifact:** line uses “refined day page (standalone file under `experts/{expert_id}/`)” verbatim.
4. `---` before `### Chronicle`.
...
Each expert keeps **`experts/<expert_id>/<expert_id>-page-template.md`** as a **short stub** linking here so existing `Template:` lines and bookmarks stay valid. Edit policy: **change the canonical file**; change stubs only when the compat paragraph itself needs revision.
