...
2. **Expert file (standalone):** **`experts/<expert_id>/<expert_id>-page-*.md`** — same **`strategy-page`** section contract; scaffold and checklist live in **[`refined-page-template.md`](refined-page-template.md)** (replace the **`{expert_id}`** token when pasting). Each expert keeps a thin compat stub **`experts/<expert_id>/<expert_id>-page-template.md`** linking the canonical file (e.g. [`ritter-page-template.md`](experts/ritter/ritter-page-template.md)). **Not** checked by `validate_strategy_pages.py` unless you **duplicate** the same logical page into a thread fence (e.g. cross-expert **`id` / `date` / `watch`**).
