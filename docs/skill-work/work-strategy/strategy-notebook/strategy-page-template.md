...
**Contract:** A **strategy-page** is a notebook page with **`### Reflection`**, **`### Foresight`**, optional **`### Appendix`**, and the **length / cadence** rules, plus a **verbatim-first body section** whose heading depends on **surface**:

1. **Thread fence (HTML comments):** `<!-- strategy-page:start … -->` … `<!-- strategy-page:end -->` in the expert’s **`thread.md`** (or monthly **`*-thread-YYYY-MM.md`**) under the correct **`## YYYY-MM`**. Use **`### Chronicle`** for the full verbatim capture (lightly cleaned and formatted)—**Chronicle** names the **thread-shaped** page. This is what [`validate_strategy_pages.py`](../../../../scripts/validate_strategy_pages.py) parses.
2. **Standalone refined page:** **`experts/<expert_id>/<expert_id>-page-*.md`** — same judgment spine (**Reflection** / **Foresight** / **Appendix**), but the verbatim-first heading is **`### Verbatim`** (not **`### Chronicle`**) so **Chronicle** stays aligned with **expert threads** and **`days.md` day blocks**. Scaffold: **[`refined-page-template.md`](refined-page-template.md)** (replace **`{expert_id}`**). Each expert keeps a thin compat stub **`experts/<expert_id>/<expert_id>-page-template.md`**. **Not** checked by `validate_strategy_pages.py` unless you **duplicate** the same logical page into a thread fence.
...
**Required sections (thread fence):** **`### Chronicle`**, **`### Reflection`**, and **`### Foresight`**. **`### Chronicle`** is normally the **full verbatim** body (expert speech / essay text), lightly cleaned—**not** a paraphrase digest. Markdown **`>`** blockquotes **inside Chronicle** are optional **lift markers**: lines you may **re-elevate** into the expert **thread** month journal (lede/closer) when composing that month; they do not change `validate_strategy_pages.py` behavior. Pre-elevation to month bookends is **operator-picked** per the **verbatim quote selection** rubric in [strategy-expert-template.md](strategy-expert-template.md#thread-template) (Thread). **`### Reflection`** / **`### Foresight`** are **operator analysis** (judgment, falsifiers, resume lines—grammatical prose when filled in); **stubs are OK** until a later pass. List dumps and machinery belong in **`### Appendix`** when used, or overflow — see below. **Standalone refined pages** swap **`### Chronicle`** for **`### Verbatim`** (same verbatim rules); see [refined-page-template.md](refined-page-template.md).
