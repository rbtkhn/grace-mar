...
|-----------|--------|-----------------|------------------|-----------------------------------|
| `islamabad-process` | Seyed Mohammad Marandi (`@s_m_marandi`) | Iranian English long-form: negotiation **process**, red lines, legitimacy register | `IRAN`, `TEHRAN`, or `Marandi` in cold | × `washington-channel` (Ritter), × `lebanon-scope` (Parsi), × `rome-ecumenical` (Pontifex / Marandi Easter) |
| `washington-channel` | Scott Ritter (`@RealScottRitter`) | U.S. **military dissent**: Hormuz **sea control**, blockade ops, Vance frame; **faith-politics** register when **Ritter** is the speaking analyst | `JDVance`, `IRAN`, or `Ritter` | × `islamabad-process`, × `hormuz-domestic` (Barnes), × `rome-invective` (split from ecumenical) |
...
| `game-theory-escalation` | Robert Pape (`@ProfessorPape`) | **Escalation Trap** / commitment ratchet on demands | `ProfessorPape` or Pape in cold | × `extension-game` (Davis), × `hormuz-domestic`, × `structural-pause`, × `jiang-xueqin` |
...
| `structural-pause` | John Mearsheimer (`@mearsheimerj`) | **Offensive realism**: security dilemma, Israel structural, great-power geometry | `MEARSHEIMER` or `Mearsheimer` in cold | × `extension-game` (Davis), × `diplomatic-institutional` (Mercouris), × `glenn-diesen` (Diesen), × `jiang-xueqin` |
| `diplomatic-institutional` | Alexander Mercouris (`@AMercouris`) | **Institutional / narrative** diplomatic read (Hormuz, Lebanon, Islamabad) | `Mercouris` or mind cite in cold | × `structural-pause` (Mearsheimer), × `glenn-diesen` (Diesen), × `islamabad-process`, × Tri-Frame [minds/](../minds/README.md) |
| `larry-johnson` | Larry Johnson (`@LarrySonar21`) | Ex-CIA / **material** and **ORBAT** emphasis: force structure, **Hormuz** geometry, **F-15/Isfahan** raid narrative reconstructions (Haiphong–Ritter roundtables) | `Johnson` or `LarryJohnson` in cold | × `washington-channel` (Ritter), × `extension-game` (Davis); see [transcript digest](../transcript-analysis-haiphong-ritter-johnson-iran-2026-04.md) |
...
| `alastair-crooke` | Alastair Crooke (`@conflictsforum`) | Former diplomat / **Levant–Islamabad** “room” and **spoiler** reads; **Conflicts Forum** org **X** (verify); **Substack**/Telegram common; often beside **Davis** in digests | `Crooke` or `conflictsforum` in cold | × `extension-game`, × `islamabad-process`, × `lebanon-scope` |
| `glenn-diesen` | Glenn Diesen (`@Glenn_Diesen`) | **Eurasia / multipolar** discourse; **non-Western** institutional / rationality frames when distinct from **`structural-pause`** (Mearsheimer) | `Diesen` in cold | × `structural-pause` (Mearsheimer), × `third-party-system`, × `game-theory-escalation`, × `jiang-xueqin` |
| `jiang-xueqin` | Jiang Xueqin (`@xueqinjiang`) | **Predictive History** / **work-jiang**: long-horizon **game-theory** & **civilization-scale** pattern reads (geo-strategy, lecture corpus); **skill-strategy** Jiang resonance — **not** work-politics pulse or generated weekly brief | `Jiang`, `Xueqin`, or `ProfessorDang` in cold | × `structural-pause` (Mearsheimer), × `glenn-diesen`, × `game-theory-escalation` (Pape); [Predictive History book](../../../../research/external/work-jiang/BOOK-ARCHITECTURE.md) |
...
- **Chas W. Freeman Jr.** — **`charles-freeman`** row has **no** **`@`** in the **Anchor** column until the **operator** pins a **canonical** **X** profile (e.g. from [chasfreeman.net](https://chasfreeman.net/)); **verify** before **ingest**-grade cite.
