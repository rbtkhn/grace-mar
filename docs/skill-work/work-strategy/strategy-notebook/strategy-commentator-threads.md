...
| `structural-pause` | John Mearsheimer | **Offensive realism**: security dilemma, Israel structural, great-power geometry | `MEARSHEIMER` or `Mearsheimer` in cold | × `extension-game` (Davis), × `diplomatic-institutional` (Mercouris), × `glenn-diesen` (Diesen), × `jiang-xueqin` |
