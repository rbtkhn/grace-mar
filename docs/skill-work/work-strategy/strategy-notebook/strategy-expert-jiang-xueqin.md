# Strategy expert — Jiang Xueqin (`jiang-xueqin`)

WORK only; not Record.

**Canonical index:** [strategy-commentator-threads.md](strategy-commentator-threads.md) — **`jiang-xueqin`** lane.

**Routing rule:** Predictive History is the sole upstream for notebook-facing Jiang ingest. PH-derived material may inform strategy-notebook only through this expert lane. Direct PH → knot routing is disallowed; route PH notes through inbox with `thread:jiang-xueqin`, then weave on operator intent.
...
- `jiang-xueqin` + `john-mearsheimer` — great-power structural analysis; both use game-theory framing to explain state behavior
- `jiang-xueqin` + `glenn-diesen` — multipolar civilizational framing; both treat Western-centric frameworks as insufficient for Eurasian dynamics
...
- `jiang-xueqin` x `robert-barnes` — civilizational game-theory vs domestic U.S. legal/liability framing; different time horizons and different planes
- `jiang-xueqin` x `scott-ritter` — long-arc mechanism analysis vs near-term operational/military detail; PH lectures rarely address current ORBAT
...
- Pull this expert into weave when:
  ...
  - The inbox has PH-derived material tagged `thread:jiang-xueqin`
...
This expert lane is the **mandatory membrane** for PH → strategy-notebook. PH material that has not been routed through `thread:jiang-xueqin` and this file may not directly shape knots.
