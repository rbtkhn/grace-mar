...
**Use:** One **expert-agnostic** falsifiable statement per row—what the **world** would have to show for the fork to be wrong—when **two or more** roster commentators have **overlapping** (converging or conflicting) claims in the same week or arc. This is **not** a duplicate ledger: per-expert receipts stay in **Logged rows** below. **Not** checked by `validate_expert_predictions.py` (that script validates **`pred_id`** rows only).

| Falsifiable prediction (topic — no named expert in the claim) | `topic_slug` | Experts who weight this fork (indicative `expert_id`s) | Where to adjudicate (`pred_id` + seams) |
|---------------------------------------------------------------|--------------|--------------------------------------------------------|----------------------------------------|
| **Hormuz commercial passage under the billed ceasefire matches a coherent regime in primaries** — either “open to commercial traffic on a coordinated route” **without** extra Iranian filters, **or** Iranian gatekeeping (commercial-only / route / approval), **or** executive “open” narrative **not** matching field/AIS—**not** all three at once without a labeled seam. | `hormuz-strait` | `davis`, `marandi`, `parsi`, `barnes` | `pred-2026-04-17-davis-hormuz-commercial-ceasefire-route` · `pred-2026-04-17-marandi-hormuz-three-conditions` · `pred-2026-04-18-parsi-hormuz-tehran-decides` · `pred-2026-04-18-barnes-hormuz-not-open-counter-trump`; seam `pred-2026-04-16-marandi-hormuz-no-toll-free` |
| **A Pakistan-track U.S.–Iran framework closes on a “days → 30–60d” calendar** from mid-April 2026 as Beltway-facing reporting claims. | `islamabad-us-iran-talks` | `parsi`, `marandi` | `pred-2026-04-17-parsi-pakistan-mediation-window`; same-week BP **Islamabad** authority seam [daily-strategy-inbox.md](daily-strategy-inbox.md) `thread:marandi` **2026-04-16** |
| **Lebanon’s near-term trajectory is durable settlement / power-shift signaling vs sectarian breakdown or civil-war-like violence** along the Washington–Beirut field axis (not merging wire “talks happened” with map-thesis violence in one object without primaries). | `lebanon-vs-nuclear-scope` | `pape`, `parsi` | `pred-2026-04-14-pape-lebanon-civil-war-fork` · `pred-2026-04-17-pape-lebanon-power-shift-nyt`; inbox `thread:parsi` Lebanon / TRT / Kent cross-refs **2026-04-17** |
| **Staged blockade shock hits dated macro checkpoints** (e.g. late-Apr → May/Jun shortage / contraction claims) as a checkable calendar—vs smooth adjustment or moved goalposts. | `sanctions-energy-oil` | `pape`, `jermy` | `pred-2026-04-16-pape-blockade-shock-calendar`; **`jermy`** when the same fork is ingested with `thread:jermy` + logistics primaries |
| **Published U.S. executive demand stacks still leave material negotiation space** for the Islamabad channel (vs ultimatum / escalation-trap read that forecloses). | `escalation-trap-coercion` | `davis`, `pape` | `pred-2026-04-17-davis-trump-ts-maximalist-slams-door`; inbox § **2026-04-16** Pape Janssen **escalation trap** `thread:pape` + `batch-analysis | 2026-04-16 | Pape × Mearsheimer lattice` |
| **Large-scale Iran–Israel shooting resumes on a short post-ceasefire fuse** (vs extended pause / different escalation shape). | `ceasefire-pause-resumption` | `marandi`, `pape`, `davis` | `pred-2026-04-16-marandi-israel-restart-war-soon`; Pape **Israel as spoiler** / stages [daily-strategy-inbox.md](daily-strategy-inbox.md) **2026-04-16** `thread:pape`; `davis` **resumption clock** lines same arc (add `pred_id` when distilled) |
| **Iran does not ship enriched uranium out** under the authority structures cited by Washington and Tehran in the same negotiation arc. | `escalation-trap-coercion` | `macgregor`, `pape` | `pred-2026-04-18-macgregor-iran-nsc-no-uranium-out`; Pape Janssen **Vance / uranium** fork same § **2026-04-16** `thread:pape` |
