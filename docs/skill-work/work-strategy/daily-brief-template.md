...
**Purpose:** One dated file: **WAP snapshot** + **work-strategy focus** + **RSS ingest** with **two relevance scores** (W = campaign/KY-4, S = product/governance/tech), then operator synthesis.
...
- `--no-fetch` — offline; WAP + strategy sections still populate from docs/gate.
...
| Section | Source |
|---------|--------|
| **1. WAP snapshot** | `get_wap_snapshot()` |
...
| **3. Lead themes** | WAP-heavy vs strategy-heavy stubs from top titles |
...
| **5–6** | Operator synthesis + WAP next actions |
