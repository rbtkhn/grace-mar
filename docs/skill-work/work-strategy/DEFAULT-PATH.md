...
Do **not** touch `days.md` unless the operator says **weave**.
...
| Letter | Option |
|--------|--------|
| **A** | Weave inbox → `days.md` (opens a `## YYYY-MM-DD` block) |
...
If the operator picks **A**, follow the weave contract: Signal / Judgment / Links minimum, then condense if over ~1200 words. If they skip, the inbox holds state for next time.
...
| [NOTEBOOK-PREFERENCES.md](strategy-notebook/NOTEBOOK-PREFERENCES.md) | Operator narrowings (minimum sections, prose register, weave rhythm) |
