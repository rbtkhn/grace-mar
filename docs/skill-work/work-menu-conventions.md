...
- **Fixed session menu (`coffee` — same A–G for work-start and signing-off; legacy hey still works):** not the 3–5 WORK pattern — use the canonical **`coffee`** fixed menu, listed **A, B, C, D, E, F, G** (seven modes; **no** close letter). **A — Today**, **B — Build**, **C — Compass** (includes work-strategy-rome), **D — Book**, **E — Steward**, **F — Xavier next**, **G — Dev next** (`workspace.md` § Next actions). **C** and **D** exit the coffee hub by default unless **`stay in coffee`**; **skills** / **meta:** say with **B**. Roles: [.cursor/skills/coffee/SKILL.md](../../../.cursor/skills/coffee/SKILL.md).
