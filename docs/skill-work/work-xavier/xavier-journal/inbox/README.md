# Xavier journal — inbox (L2 operator context)

Drop **repo-local** markdown here so [`scripts/xavier_journal_ob1_digest.py`](../../../../../scripts/xavier_journal_ob1_digest.py) can merge it into the generated day file under **Operator context (ingested)**.

**Rolling buffer:** Prefer capturing rough notes in [../daily-xavier-journal-inbox.md](../daily-xavier-journal-inbox.md) during the day; at **`dream`**, fold into **`YYYY-MM-DD.md`** here (rolling file is not auto-cleared; prune when long — see that file). See [xavier-journal README](../README.md#daily-inbox-rolling-accumulator).
...
Optional last line in a day file, e.g. `→ Promote to history: phase-1 foundation shipped`. The digest does not parse this in v1; use it as a reminder when updating [`work-xavier-history.md`](../../work-xavier-history.md).
