# Xavier journal — synthesis sources

**Territory:** WORK / operator coaching — not Xavier’s Record, not companion gate merge.

This document describes **what feeds** the generated day file from [`scripts/xavier_journal_ob1_digest.py`](../../../../scripts/xavier_journal_ob1_digest.py) and **how recursive learning** stays visible.
...
| **L2a′** | *(Workflow only)* [daily-xavier-journal-inbox.md](daily-xavier-journal-inbox.md) folded at **`dream`** into **`inbox/YYYY-MM-DD.md`** (rolling buffer not auto-cleared; prune when long — see inbox file) — not read by the digest directly | Becomes part of L2a after fold |
...
**Contract:** Nothing is pulled from Cursor’s cloud automatically. To **capture transcript discussion and geopolitical ideas** in each journal page, use **`--full-day-synthesis`** (or set env **`XAVIER_JOURNAL_FULL_DAY_SYNTHESIS=1`**) so the digest pulls **strategy-notebook + session-transcript** for the same calendar day, and/or add notes under **`inbox/`**.
...
| `XAVIER_JOURNAL_FULL_DAY_SYNTHESIS=1` | Same as `--full-day-synthesis` (unset when you want flags off). |
...
1. **Promotion:** Strong days can motivate a milestone in [`work-xavier-history.md`](../work-xavier-history.md); optional human line `→ Promote to history:` in inbox (not parsed by the script in v1).
...
3. **Rolling review:** Every few days, consider [harvest](../../../../.cursor/skills/harvest/SKILL.md) or a narrative pass using recent `xavier-journal/*.md` as context so the log stays **read**, not write-only.
...
6. **Rolling daily inbox:** Optional [daily-xavier-journal-inbox.md](daily-xavier-journal-inbox.md) → fold at **`dream`** into **`inbox/YYYY-MM-DD.md`** (becomes L2a); buffer prunes by length, not mandatory nightly reset — see README § *Daily inbox (rolling accumulator)*.
...
- Same-day anchor line from **work-xavier-history.md** (parse or manual).
