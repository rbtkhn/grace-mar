# work-xavier and repo CI / labels

## PR labels (required on `main`)

**work-xavier–only** changes (`docs/skill-work/work-xavier/**`, including `work-dev-mirror/` and `work-politics-mirror/`) should use GitHub label **`lane/work-xavier`**. See [.github/pull_request_template.md](../../../.github/pull_request_template.md).
...
Xavier’s **cognitive fork** lives in **her instance repo**; durable identity changes there use **her** gate and merge script — not silent edits from grace-mar `work-xavier` files.

There is no work-xavier–specific gate paste CLI in this repo; use the standard staging tools if a milestone is explicitly gated.

## Summary

| Change | Label |
|--------|--------|
| Xavier advisor docs, mirrors, runbooks, BrewMind WORK files | `lane/work-xavier` |
