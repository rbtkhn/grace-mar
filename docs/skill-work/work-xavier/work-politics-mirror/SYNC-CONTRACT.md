...
Each `good morning`, run a `work-politics` relevance scan and return:
...
- No direct writes to `users/xavier/self.md` during sync.
- If sync insight implies identity update, stage via `users/xavier/recursion-gate.md`.
