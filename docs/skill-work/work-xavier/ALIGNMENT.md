...
|-------|---------------------------|------------------------|-----------------------------------------------|
| Role | Blueprint | Reference instance `users/grace-mar/` + **work-xavier** advisor module | Her fork — **`users/xavier/`** (or template ID) **in her repository** |
| Record | `users/_template/` scaffold only | Live grace-mar Record | Her Record — **hosted outside** grace-mar |

**grace-mar:** Operator runs the **work-xavier** project (this folder) to advise Xavier; mirrors and runbooks live here.
