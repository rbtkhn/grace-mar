...
*Filename `GOOD-MORNING.md` is legacy; grace-mar operator activation is **coffee** (work-start vs **signing-off** intent — same **A–G** menu; **C — Compass** includes work-strategy-rome; **F** / **G** = xavier next / dev next), with legacy **hey** still accepted — mirror the current vocabulary in chat.*
