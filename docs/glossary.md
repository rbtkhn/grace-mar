...
| **Record** | Companion-owned **canonical data** with **four surfaces**: (1) **SELF** — identity + **SELF-KNOWLEDGE** (`self.md`, IX-A/B/C); (2) **SELF-LIBRARY** — **reference-facing** governed library (`self-library.md`), including **CIV-MEM** as a sub-library; (3) **SKILLS** (`skills.md`); (4) **EVIDENCE** (`self-evidence.md`). Identity ≠ library: corpora live in SELF-LIBRARY, not in IX-A. Gate applies to merges into these surfaces. See [boundary-self-knowledge-self-library.md](boundary-self-knowledge-self-library.md). |
