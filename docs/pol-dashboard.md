...
**Daily rhythm cards:** The dashboard includes a static **A–E** reference strip (**coffee** — work-start and signing-off (same menu); legacy **hey** still works; five options; **no** micro-hints row under the list) plus links to [work-menu-conventions.md](skill-work/work-menu-conventions.md), [coffee SKILL](../.cursor/skills/coffee/SKILL.md), and [suggest_morning_forks.py](../scripts/suggest_morning_forks.py). No server logic — glanceable only.
