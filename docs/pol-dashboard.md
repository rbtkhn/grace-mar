...
**Daily rhythm cards:** The dashboard includes a static **A–G** reference strip (**coffee** — work-start and signing-off (same menu); legacy **hey** still works; seven modes; **no** close letter; **C — Compass** includes work-strategy-rome; **F** / **G** = xavier next / dev next) plus links to [work-menu-conventions.md](skill-work/work-menu-conventions.md), [coffee SKILL](../.cursor/skills/coffee/SKILL.md), and [suggest_morning_forks.py](../scripts/suggest_morning_forks.py). No server logic — glanceable only.
