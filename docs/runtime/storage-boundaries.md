...
**Supplements** [memory-retrieval.md](memory-retrieval.md) (normative governance rules). This file covers **ledger placement and ambient-write safety** only.
...
3. **No silent promotion** — Runtime JSON never merges into the Record without the gate pipeline (see normative doc).
...
`tests/runtime/test_log_observation.py` and `tests/runtime/test_no_ambient_file_writes.py` assert loggers do not write outside the observations tree or create ambient instruction files.
