...
When an operator or agent runs a **template + boundary audit** (e.g. operator-cadence hey **A**), the narrative should close with **specific upstream / downstream recommendations for reconciliation code** — `scripts/`, validators, CI, hooks — not only doc drift. Spec: [work-companion-self/README — Reconciliation code audit](skill-work/work-companion-self/README.md#reconciliation-code-audit-upstream-and-downstream).
