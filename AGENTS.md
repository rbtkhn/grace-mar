...
**SKILLS (Record-bound):** **self-skill-think**, **self-skill-write**, **self-skill-work**, and **self-skill-steward** (STEWARD — governance literacy at the gate; optional split template; **not** merge authority) are capability surfaces under `users/[id]/`; see [id-taxonomy.md](docs/id-taxonomy.md), [skills-modularity.md](docs/skills-modularity.md). **Skill lifecycle (discovery ladder):** `skills-portable/README.md` — pointer → draft → listed. **Skill validation:** `python3 scripts/validate_skills.py`.
