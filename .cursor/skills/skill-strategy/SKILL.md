...
- **Fold** escalated captures under **Judgment** as **audience dynamics** and **register shift** unless the operator runs **+ verify** on claims that could ship as **Links** facts.
...
- **Tag the plane** when a load-bearing claim draws on a watch thread — in **Judgment** or **Links** (which MFA, which Kremlin URL, which WH readout), not only “analysts say.”
...
**Relation to `dream`:** Daytime **`strategy`** passes **produce** notebook judgment. **`dream`** does **not** own strategy-notebook production — fold when **you** run **`strategy`** or direct **fold**; `dream` may only surface optional calendar **FYI** (`strategy_notebook_missing_day_headers`). See [STRATEGY-NOTEBOOK-ARCHITECTURE.md](../../../docs/skill-work/work-strategy/strategy-notebook/STRATEGY-NOTEBOOK-ARCHITECTURE.md) § *Entry model* and [.cursor/skills/dream/SKILL.md](../dream/SKILL.md) § *Strategy notebook*.

**Seal with `self-skill-write`:** When a block becomes the **finished** notebook entry for that fold, **`strategy`** (structure) and **`self-skill-write`** (linguistic layer — [skills-modularity](../../../docs/skills-modularity.md)) **converge** on the same markdown. That seal occurs in a **`strategy`** session (or when the operator explicitly folds in another thread); **`dream`** is not the default seal point.
...
| **Fold** | **Explicit** operator direction (`fold`, **`strategy`** session), or same-thread request during `dream` | Active month `chapters/YYYY-MM/days.md` under **`## YYYY-MM-DD`** (or episodic `##` per architecture) | **Block fragment** per § *Daily block extension* and [Daily entry template](../../../docs/skill-work/work-strategy/strategy-notebook/STRATEGY-NOTEBOOK-ARCHITECTURE.md#daily-entry-template) — **academic** prose. **Do not** append or merge into `days.md` until Fold unless explicitly directed. |
...
2. **Daily block fragment shape** — Use **only** when the operator **explicitly** directs a same-session **`days.md`** update or at **Fold**. Output **only** content that can be appended under the correct **`## YYYY-MM-DD`** block in `chapters/YYYY-MM/days.md`, in **section-sized** edits per § *Daily block extension*.
...
Before any **Fold** or **`days.md`** write:
...
**Write only the section(s) being updated** in this pass. **Do not** regenerate the entire dated block unless the operator explicitly asks for a **full rewrite** or **condense** pass. **Exception:** an explicit **“fold inbox into today”** (or equivalent) may update **multiple** sections in one Fold.
...
**Do not** place directly into the daily **spine** unless compressed to judgment value: transcript walls, multi-paragraph web dumps, lens-by-lens essays, large comparison tables, quote-heavy extracts. **Lens** depth stays in chat / inbox until Fold compresses or links — see [Post-entry lens offer](#post-entry-lens-offer) and NOTEBOOK-PREFERENCES (**notebook thin**).

## Pre-write checklist

Before any **Capture** or **Fold** file write, confirm:
