...
**Signing-off intent** (end of session, wrapping the day, stepping away) uses the **same** **`coffee`** trigger and the **same** **A–I** menu (**order A, B, C, D, E, G, H, I, F** — **F** closes). There is **no** separate closeout branch or closeout-only menu item. **Step 1** switches to handoff-weighted output when intent is signing off; **Step 2** is unchanged. Choosing **any** letter **is** the transition out of the hub (same as work-start), except **F** which only closes without picking work.
...
The operator may say **`coffee`** **more than once per calendar day** for reorientation. This is normal. Each new `coffee` runs Step 1 again and starts a new A–I cycle. If **`coffee`** arrives before the prior session reached **F**, treat it as a **reorientation restart**: run Step 1 again, then offer a fresh A–I menu.

### Step 2 — Multiple choice (required; always A–I + F)

Immediately **after** Step 1 content, output the fixed **coffee** menu: **nine** lettered options **A through I**, presented **A, B, C, D, E, G, H, I, F** (**F** closes). **Do not** omit **F**. **I** is **always** [work-strategy-rome](../../../docs/skill-work/work-strategy/work-strategy-rome/README.md) (ROME-PASS and related artifacts).
...
- **E.** Work-dev *or* work-strategy *or* work-politics — one lane + one recommendation; **or** system pick when signing off and **E** with no sub-lane (see [menu-reference.md](../../../docs/skill-work/work-coffee/menu-reference.md)). **Rome / Vatican strategy** → **I**, not a substitute under **E** alone.
- **G.** work-jiang / Predictive History (exits to normal workflow)
- **H.** Skills / meta pipeline
- **I.** work-strategy-rome — ROME-PASS, manifest, exemplars, thesis stubs; **one** develop step (exits to normal workflow)
- **F.** End coffee — close session without choosing work

**Re-offer rules:** After **A, B, C, D, or H**, re-offer the full menu. After **E** with a concrete sub-lane, **G**, or **I**, **exit to normal workflow** unless the operator says **`stay in coffee`**. **F** formally closes the session.

For full **A–I** definitions, signing-off add-ons per letter, and the companion survey track: see [menu-reference.md](../../../docs/skill-work/work-coffee/menu-reference.md).
...
- `docs/skill-work/work-coffee/menu-reference.md` — full A–I definitions, cadence, survey track, signing-off add-ons
