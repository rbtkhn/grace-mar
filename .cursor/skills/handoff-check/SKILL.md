...
description: Run operator_handoff_check.py for RECURSION-GATE pending, Predictive History night closeout, commits, worktree noise, re-entry prompt — read-only. On **coffee** with **signing-off** intent, this script (or `operator_coffee.py --mode closeout`) is **coffee Step 1**; **Step 2** is the **same** fixed **A–E** menu as work-start — **E — Steward** without gate/template split → **system pick**. See [coffee/SKILL.md](../coffee/SKILL.md) and [menu-reference — signing-off intent](../../../docs/skill-work/work-coffee/menu-reference.md#signing-off-intent). Also use when resuming work or checking safe-to-ignore before commit/push.
---
...
**Preset — signing-off `coffee`:** When the operator says **`coffee`** with **signing-off** intent (session end, wrapping the day; legacy **`hey`** still works), the agent runs **signing-off Step 1** (this command + short summary paragraph) then the **same** **A–E** menu as work-start. **E — Steward** with **no** gate vs template split named → **system pick** (see [menu-reference — signing-off intent](../../../docs/skill-work/work-coffee/menu-reference.md#signing-off-intent)). On follow-up turns, **A, B, or E** re-offer the full menu by default; **C** and **D** exit to normal workflow unless **`stay in coffee`**. **No close letter** — exit the hub by normal workflow or **no menu**.

**Work-start `coffee`:** After **A, B, or E**, **re-offer the full A–E menu** by default. **C — Compass** and **D — Book** exit to normal workflow unless **`stay in coffee`** — see [coffee/SKILL.md](../coffee/SKILL.md).
