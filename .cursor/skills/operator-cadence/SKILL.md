...
preferred_activation: hey
description: "Grace-Mar operator cadence and tempo. Single trigger: hey (modifiers: hey light, minimal, survey). Work-start hey: Step 1 = operator_daily_warmup + harness + branch snapshot. Closeout hey (signing off in same message): Step 1 = operator_handoff_check + summary + branch snapshot. Read-only planning; Step 2 = fixed A–H until F. G = work-jiang/PH; H = skills/meta. Menu in body."
---

# Operator cadence

**Preferred activation (operator):** say **`hey`** only. **Work-start** vs **closeout** is **intent in the same message** (e.g. reorientation vs signing off), not a second trigger phrase — one skill, two Step 1 shapes.
...
|-----|------|-------------|
| **Monday** | **Full** | Complete [“Hey”](#hey--start-here-two-steps) flow: operator + harness + branch snapshot in Step 1. **Internet intel** (Polymarket, independent polls, Massie X): **not** Step 1 — run on **menu E → work-politics** (full Monday weight) or explicit request. **Daily brief:** **menu C** only. |
...
If the operator says **“hey”** on a **Sunday**, default to **week-ahead** mode unless they ask for the full Monday stack. Still run **Step 1** scaled to that mode, then **Step 2** with the full **A–H** menu (labels can be shorter; meanings unchanged).

### Explicit phrases (override default cadence when stated)

**“Hey light”** (or clear equivalent):

- Run **`operator_daily_warmup.py`** and, when instance state matters, **`harness_warmup.py`**.
- **Internet intel** (Polymarket, polls, Massie X) is **never** Step 1 — same as full hey; choose **E → work-politics** when you want it (or ask explicitly). **Light** keeps **branch snapshot** compact (one line unless multiple branches).
...
- Still output **full Step 2 A–H**. On later turns, **A, B, C, D, E, G, or H** then **re-offer the full A–H menu** until **F** (same as full/light hey). **Track B:** **minimal** repository hygiene — extend the Step 1 branch line with `git status -sb` + **one** next action (or “clean”); no unrelated sweeps.

**“Hey survey”** (or **“hey + survey”** / clear equivalent):

- Run **Step 1** using the same **[cadence by weekday](#cadence-by-weekday)** or **[explicit phrases](#explicit-phrases-override-default-cadence-when-stated)** as if they had said plain **hey** (they may combine with **hey light** or **minimal** — apply both: thin work-politics steps *and* survey intent).
...
- **Step 2** remains the **same fixed A–H** menu (do not drop letters). When the operator chooses **E**, **default** the **work-politics** sub-lane and run **[Companion survey track](#companion-survey-track-hey)** for that turn (unless they explicitly steer **E** to work-dev or work-strategy in the same message, e.g. “E work-dev only”).
...
<a id="hey--start-here-two-steps"></a>
...
The operator may say **hey** **more than once per calendar day** whenever they need **reorientation** toward the most productive tasks — not only at literal day start. **Each** new **hey** runs **Step 1** again (at the cadence or explicit phrase for that message) and starts a **new** A–H cycle. **F** closes only the **current** hey session in this thread; it does **not** imply a one-per-day limit.

**Habit (optional):** For a **second or later** pass the same day, **hey light** / **minimal** shortens scripts and branch snapshot. Plain **hey** again still **does not** auto-run internet intel in Step 1 — for refreshed markets / Massie X they choose **E → work-politics** (or ask explicitly in the message).

If **hey** arrives **before** the prior hey session reached **F**, treat it as a **reorientation restart:** run Step 1 again, then offer a fresh A–H menu (use thread context to label **E** sub-lanes).
...
3. **Polling + Polymarket + web polls + Massie X (deferred — menu E / work-politics):** Do **not** fetch Polymarket, run web search for polls, or run the Massie X skill in Step 1 **unless** the operator’s **same message** explicitly requests that intel (e.g. “hey + Polymarket” / “run KY-4 markets”) — then run the stack **in Step 1** for that turn. Otherwise **one line** in the warmup reply: e.g. “KY-4 markets + polls + Massie X: choose **E → work-politics**.” When the operator chooses **[E / work-politics](#hey--multiple-choice-ah-required)** and the track is **not** **[hey survey](#explicit-phrases-override-default-cadence-when-stated)** defaulting to **[Companion survey](#companion-survey-track-hey)** only, run [polling-and-markets.md](../../../docs/skill-work/work-politics/polling-and-markets.md) **and** `.cursor/skills/politics-massie/SKILL.md` — **Monday / full** = full pass; **Tue–Fri** = compact (top story links or one draft) unless the operator asks for more. **Hey survey** + **E** / work-politics **survey** path: run the **survey** track **without** the Polymarket / poll / Massie web stack **unless** the operator asks for both in that message. Update **`Last checked`** in `polling-and-markets.md` when you materially refresh numbers.
4. **Branch snapshot (operator default):** Run `git status -sb` and `git branch -vv` (read-only). Deliver **one short plain-language block** in the warmup reply: if only `main` (or clean tracking), say **branch hygiene: clean**; if other branches exist, classify **merged/stale vs active** per [git-branch-hygiene.md](../../../docs/skill-work/work-dev/git-branch-hygiene.md) and give **one prescribed action** or **no action**. **Not menu A** — A is template/boundary audit; this is **local git** hygiene only. **Hey light / minimal:** one line from `git branch -vv` unless multiple non-`main` branches or ambiguous tracking — then the short paragraph.
...
Immediately **after** Step 1 content, output the fixed **A–H** menu (see [Hey — multiple choice (A–H required)](#hey--multiple-choice-ah-required)). **List order:** **A, B, C, D, E, G, H, F** so **F** (end session) is last. **Do not** omit **F**.

When the operator later sends **A**, **B**, **C**, **D**, **E**, **G**, or **H** (or equivalent), **execute that track** without re-running Step 1 unless they say **hey** again. **After** that track’s content, **always output the full A–H day menu again** (same roles as [Hey — multiple choice](#hey--multiple-choice-ah-required)). The hey session stays **open** until **F** — do not skip the menu on non-**F** turns.

When the operator sends **F** (or “end hey session” / clear equivalent): **formally close** the hey session — short acknowledgment (one or two sentences). **Do not** run Step 1 automated stack on subsequent turns until the next **hey** (which may be **later the same day** for reorientation). **Work-start `F`** does not require handoff — only a **closeout** **hey** runs `operator_handoff_check.py` in Step 1 (see [Hey — closeout](#hey-closeout-session-end)).

---

## Hey — multiple choice (A–H required)

The **first** hey reply ends **Step 2** with **exactly eight options — A through H**, presented **A, B, C, D, E, G, H, F** (**F** closes). **Each follow-up** after the operator chooses **A**, **B**, **C**, **D**, **E**, **G**, or **H** must also end with the **same full A–H menu** (until **F** closes the session). Wording may vary; **roles must not**.
...
| **F** | **End hey → normal workflow** | **Formally closes** the hey session (see **Step 2** under [Hey = start here](#hey--start-here-two-steps)). Transition to normal chat/work until the next **hey**. Does **not** by itself imply closeout handoff — that comes from a **closeout** **hey**. |

**Example shape (E shows chosen sub-lane when picked):**

```markdown
**Hey — pick one:** *(order: A–E, then G, H, then F)*
...
- **E.** **Work-dev *or* work-strategy *or* work-politics** — **one** lane + one recommendation *(work-politics = KY-4 + Polymarket + polls + Massie X **here**, not Step 1; survey-only path skips that stack; work-dev = source spot-check **here** only; Jiang = **G**; skills = **H**)*
- **G.** **work-jiang / Predictive History** — PH momentum block, `work-jiang.md`, sparks, optional `warmup_jiang_pulse.py` *(one next step)*
...
- **F.** **End hey** — close session; normal workflow until next “hey”
```
...
## Shared A–C (work-start and closeout under hey)

**Work-start** and **closeout** use the **same** meanings for **A**, **B**, and **C**. When the operator chooses **A**, **B**, or **C** during a **closeout** pass, execute the same track as the matching row in **[Hey — multiple choice (A–H required)](#hey--multiple-choice-ah-required)**.
...
## Shared D (work-start and closeout under hey)

**Work-start** and **closeout** use the **same** meaning for **D** — **RECURSION-GATE**. When the operator chooses **D** during **closeout**, execute the same track as **[hey D](#hey--multiple-choice-ah-required)** (deepen from handoff **`## RECURSION-GATE (pending)`** when helpful).

---

<a id="shared-g-h-morning-and-night"></a>

## Shared G-H (work-start and closeout under hey)

**Work-start** and **closeout** use the **same** meanings for **G** and **H**. When the operator chooses **G** or **H** during **closeout**, execute the same track as the matching row in **[Hey — multiple choice (A–H required)](#hey--multiple-choice-ah-required)**.

- **G during closeout:** Ground primarily in handoff **`## Predictive History — night closeout`** (already in Step 1 paste) and deepen Jiang / PH closeout per the hey **G** row — do not duplicate **closeout E**’s one-line “fill from handoff” unless **E** is the chosen letter.
- **H during closeout:** Same as work-start **H**; skill discovery backlog is **not** Record and **not** gate merge.

---

## Companion survey track (hey)

**When:** Operator chose **hey E** with **work-politics** sub-lane and the pick is survey — or they began with **[hey survey](#explicit-phrases-override-default-cadence-when-stated)** and then chose **E** (default survey under **E** / work-politics). **During closeout:** survey still available under **E** (system pick), not under **A–D**.
...
<a id="hey-closeout-session-end"></a>
...
5. **Branch snapshot (same as hey step 4):** `git status -sb` and `git branch -vv`; **one plain sentence** if any non-`main` branch exists or tracking looks stale — else **skip** with “branch hygiene: clean.” Prescriptive rules: [git-branch-hygiene.md](../../../docs/skill-work/work-dev/git-branch-hygiene.md). **Not menu A.**

Full spec: [`.cursor/skills/handoff-check/SKILL.md`](../handoff-check/SKILL.md).

### Step 2 — Multiple choice (required; always A–H)

Immediately **after** Step 1, output the fixed **A–H** menu (see [Hey — closeout — multiple choice](#hey-closeout-menu)). **List order:** **A, B, C, D, E, G, H, F**. **Do not** omit **F**.

When the operator later sends **A**, **B**, **C**, **D**, **E**, **G**, or **H** (or equivalent), **execute that track** without re-running **Step 1** (`operator_handoff_check.py`) unless they ask for a **fresh handoff** or open a new **closeout** **hey**. **After** that track’s content, **always output the full A–H menu again** (same roles as [Hey — closeout — multiple choice](#hey-closeout-menu)). The closeout pass stays **open** until **F** — do not skip the menu on non-**F** turns.

When the operator sends **F** (or “end closeout” / “end session” / clear equivalent): **formally close** the closeout pass — short acknowledgment (one or two sentences). **Do not** re-run the full closeout **Step 1** stack on subsequent turns until the next **hey** that opens closeout again. **Next** **hey** uses operator intent again (work-start vs closeout).

**Distinctions**

- **Work-start `F`:** ends the **work-start hey session** only; day may continue.
- **Closeout `F`:** ends the **closeout** pass; normal chat can continue, but **do not** treat the thread as still in closeout for rerunning handoff until the next **closeout** **hey**.

---

<a id="hey-closeout-menu"></a>

## Hey — closeout — multiple choice (A–H required)
...
**A–D** match work-start hey — see **[Shared A–C (work-start and closeout under hey)](#shared-ac-morning-and-night)**, **[Shared D (work-start and closeout under hey)](#shared-d-morning-and-night)**, and the **[hey table](#hey--multiple-choice-ah-required)**. **G** and **H** — see **[Shared G-H (work-start and closeout under hey)](#shared-g-h-morning-and-night)**.

| Letter | Role | What it means when chosen |
|--------|------|---------------------------|
| **A** | **Template + boundary audit** | **Same as hey A** — [row above](#hey--multiple-choice-ah-required). |
| **B** | **Repository hygiene** | **Same as work-start B** — [row above](#hey--multiple-choice-ah-required); ground in **handoff** + closeout Step 1 branch snapshot. |
| **C** | **Daily brief** | **Same as work-start C** — [row above](#hey--multiple-choice-ah-required); generator + §1d **only when chosen** (often today’s file before sign-off). |
| **D** | **RECURSION-GATE** | **Same as work-start D** — [row above](#hey--multiple-choice-ah-required); use handoff **`## RECURSION-GATE (pending)`** as primary grounding. |
| **E** | **System pick — productive or hygiene** | **Closeout extras** — **not** duplicating **A–D**, **G**, or **H**. **Prefer B** for full repo hygiene, **C** for daily brief, **D** for gate queue review, **G** for Predictive History / Jiang deep-dive from handoff, **H** for skill backlog / portable sync / extract-skill. **Quick boundary / integrity** — fork isolation / leakage or **`validate-integrity.py --user grace-mar`** if handoff flagged drift. **Also:** work-strategy / work-politics next step without duplicating **D**, **companion survey** ([survey track](#companion-survey-track-hey)), commit/push grouping, derived-export refresh, weekly-brief carryover, work-politics / `@usa_first_ky` queue. One clear recommendation + optional “if you only have 5 minutes.” Read-only unless operator asks to implement. |
| **G** | **work-jiang / Predictive History** | **Same as work-start G** — [row above](#hey--multiple-choice-ah-required); ground in handoff **`## Predictive History — night closeout`** (Jiang lane, Spark / `rebuild_all`, `session-log.md` / `work-jiang.md`). |
| **H** | **Skills / meta pipeline** | **Same as work-start H** — [row above](#hey--multiple-choice-ah-required). |
| **F** | **End closeout** | **Formally closes** the closeout pass (see **Step 2** under [Hey — closeout](#hey-closeout-session-end)). Does **not** run work-start Step 1; next **hey** uses operator intent again. |

**Example shape (E shows one system pick when chosen):**

```markdown
**Hey — closeout — pick one:** *(order: A–E, then G, H, then F)*
...
- **F.** **End closeout** — close the closeout pass; no rerun handoff until next **closeout** **hey**
```
...
- **Step 2 — A–H menu** (required for “hey”): fixed **A–H** as in [Hey — multiple choice (A–H required)](#hey--multiple-choice-ah-required)
- **Closeout hey:** handoff script output + summary (**Step 1**), then **Step 2 — A–H** as in [Hey — closeout — multiple choice](#hey-closeout-menu) (**A–D** same as work-start; **G–H** shared; see [Shared A–C](#shared-ac-morning-and-night), [Shared D](#shared-d-morning-and-night), [Shared G-H](#shared-g-h-morning-and-night))
