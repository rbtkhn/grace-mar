...
- **WARRANT (optional):** For personality patterns, developmental claims, or context-dependent knowledge, include a warrant — the unstated assumption that, if changed, would mean this entry should be revisited. Skip for straightforward facts or stable preferences where no expiration condition exists.
...
- **Identity vs library (staging):** IX-A is for **identity-facing** lines (who she is, what she cares about, compact facts). If the merge text would be a **domain dump**, long reference excerpt, or points at **CIV-MEM / civilization-memory paths**, do **not** use default `SELF_KNOWLEDGE_ADD` for that content — the operator should stage **library** work with `proposal_class: CIV_MEM_ADD` or `SELF_LIBRARY_ADD` and keep IX-A to a short identity hook if needed. When the exchange only supports a short identity line, stage that; do not paste corpus-scale material into `suggested_entry`.
...
warrant: <optional — the assumption that, if changed, would mean this entry should be revisited. E.g., "holds while current school environment persists" or "assumes limited self-regulation strategies." Omit for straightforward facts or preferences with no expiration condition.>"""
