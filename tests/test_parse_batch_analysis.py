...
"""
    ...
    assert set(r["expert_ids"]) == {"scott-ritter", "daniel-davis"}
    assert r["sources"]["crosses"] == ["scott-ritter", "daniel-davis"]


def test_merge_duplicate_label_unions_ids() -> None:
    text = """
`batch-analysis | 2026-04-14 | Ritter × Davis | short stub **crosses:scott-ritter+daniel-davis**`
...
"""
    ...
    refs = doc["batch_analysis_refs"]
    assert len(refs) == 1
    assert set(refs[0]["expert_ids"]) == {"scott-ritter", "daniel-davis"}
    assert "long prose" in refs[0]["raw"]


def test_upstream_threads_medium() -> None:
    text = """
`X | cold: a // hook | https://x.com/a | verify:x | thread:daniel-davis`
...
"""
    ...
    assert r["sources"]["upstream_verify"] == ["daniel-davis"]
    assert "daniel-davis" in r["expert_ids"]
...
def test_thread_on_batch_line_high() -> None:
    text = """
`batch-analysis | 2026-04-15 | Mercouris note | thread:alexander-mercouris **15 Apr** strand`
"""
    doc = parse_inbox_text(text)
    r = doc["batch_analysis_refs"][0]
    assert r["confidence"] == "high"
    assert r["sources"]["thread_in_line"] == ["alexander-mercouris"]
