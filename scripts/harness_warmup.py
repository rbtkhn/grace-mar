...
def main() -> int:
    ...
    if args.compact:
        ...
        bits.append(f"[Grace-Mar warmup · {args.user} · {ts} · territory={args.territory}]")
        bits.append(
            f"Pending ({args.territory}): {pending_n} (work-politics {wap_n} · Companion {comp_n} total)."
        )
        if pending_list:
    ...
    lines.extend(
        [
            ...
            "",
            f"- **RECURSION-GATE — pending ({args.territory}):** {pending_n} _(work-politics {wap_n} · Companion {comp_n} — use `--territory wap` or `companion`)_",
        ]
    )
    if pending_list:
        lines.append("")
