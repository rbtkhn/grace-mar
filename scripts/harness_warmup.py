...
def _pending_candidates(pr_content: str, territory: str = "all") -> list[tuple[str, str]]:
    """Return [(CANDIDATE-id, summary_one_line), ...] for status: pending. territory: all|work-politics|companion."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="all",
        help="Pending lens: work-politics (or wap/wp) | companion | all",
    )
    parser.add_argument(
        "--fresh-judge",
        action="store_true",
    ...
    if args.compact:
        ...
        bits.append(
            f"Pending ({territory}): {pending_n} (work-politics {wap_n} · Companion {comp_n} total)."
        )
        if pending_list:
    ...
    lines.extend(
        [
            ...
            "",
            f"- **RECURSION-GATE — pending ({territory}):** {pending_n} _(work-politics {wap_n} · Companion {comp_n} — use `--territory work-politics` or `companion`)_",
        ]
    )
    if pending_list:
        lines.append("")
