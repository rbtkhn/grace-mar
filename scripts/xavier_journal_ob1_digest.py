...
lines. See docs/skill-work/work-xavier/xavier-journal/README.md.
...
Usage:
  ...
  TZ=America/New_York python3 scripts/xavier_journal_ob1_digest.py --catch-up-from-last-dream --write
  python3 scripts/xavier_journal_ob1_digest.py --full-day-synthesis --write
  XAVIER_JOURNAL_FULL_DAY_SYNTHESIS=1 python3 scripts/xavier_journal_ob1_digest.py --write
...
_RE_TITLE_DAY = re.compile(r"^#\s+Xavier journal — Day\s+(\d+)\s*$", re.MULTILINE)
...
def main() -> None:
    ...
    if os.environ.get("XAVIER_JOURNAL_FULL_DAY_SYNTHESIS", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
