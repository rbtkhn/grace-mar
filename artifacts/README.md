...
| `artifacts/external-codex/` | **`scripts/build_external_codex_neighborhood.py`** | **Derived** structural-neighborhood **JSON** + optional **`.neighborhood.md`** companion for paths inside `research/repos/civilization_memory` (checkout); [bucket README](external-codex/README.md), [doc](../docs/skill-work/work-dev/external-codex-explorer.md); schema [`external-codex-neighborhood-report.v1.json`](../schema-registry/external-codex-neighborhood-report.v1.json); **Default:** generated `*.json` / `*.neighborhood.md` gitignored — committed **`examples/`** only. |
