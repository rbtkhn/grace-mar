...
| 2026-04-11 | repo-feedback-prompt | `repo feedback prompt`, `OSS feedback prompt`, `constructive GitHub prompt` — verify README/skill/API docs, gap table + leverage filter, short maintainer issue/DM | **Promotion plan:** criterion = **≥2** public repos with **full** verify→prompt pass + usable output; **reuses:** [missouri](https://github.com/chrisvoncsefalvay/missouri) (adversarial-trust doc gap), [agent-matrix](https://github.com/l0r3zz/agent-matrix) (comparative tone + README nits). **Next:** move draft → `skills-portable/repo-feedback-prompt/SKILL.md`, `manifest.yaml`, `sync_portable_skills.py --verify`, mark row `*(promoted)*`. Draft: [`_drafts/repo-feedback-prompt/SKILL.md`](_drafts/repo-feedback-prompt/SKILL.md) |
