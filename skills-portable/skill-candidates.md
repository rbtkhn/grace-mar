...
| 2026-04-11 | repo-feedback-prompt | `repo feedback prompt`, `OSS feedback prompt`, `constructive GitHub prompt` — verify README/skill/API docs, gap table + leverage filter, short maintainer issue/DM; session: Missouri upstream feedback arc | [`_drafts/repo-feedback-prompt/SKILL.md`](_drafts/repo-feedback-prompt/SKILL.md); promote after second external repo reuse → `skills-portable/` + manifest |
