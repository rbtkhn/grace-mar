...
Optional, file-based, **runtime-only**, **non-canonical**. Not a live Letta integration. No Letta API calls and no required Letta SDK.
...
A bare `python3 scripts/runtime/letta_import_summary.py` uses the same example file by default.

## See also

- [runtime complements doctrine (membrane)](../../../docs/runtime/runtime-complements.md)
- [LETTA-ADAPTER-CONTRACT](LETTA-ADAPTER-CONTRACT.md) (contract) · [letta-seed.example.json](letta-seed.example.json) · [letta-session-summary.example.json](letta-session-summary.example.json)
